...
const SOCIETY_POIS={
 paris:[
  ...
  {type:'clinic',name:'Cabinet Médical des Jardins',cx:-1,cz:1,x:-1*CHUNK+43,z:1*CHUNK+13}
 ],
 belle_rive:[
  ...
  {type:'clinic',name:'Cabinet Médical du Littoral',cx:1,cz:-1,x:1*CHUNK+43,z:-1*CHUNK+13}
 ],
 saint_roch:[
  ...
  {type:'clinic',name:'Cabinet Médical des Forges',cx:-1,cz:1,x:-1*CHUNK+43,z:1*CHUNK+13}
 ],
 valmont:[
  ...
  {type:'clinic',name:'Hôpital de Valmont',cx:2,cz:-1,x:2*CHUNK+43,z:-1*CHUNK+13}
 ],
 montfleur:[
  ...
  {type:'clinic',name:'Hôpital Universitaire de Montfleur',cx:1,cz:-1,x:1*CHUNK+43,z:-1*CHUNK+13}
 ]
};
...
// V22.9 — capacités opérationnelles finies. Les unités engagées ne peuvent pas être dupliquées.
const EMERGENCY_SERVICE_CAPACITY={
 paris:{fire:3,samu:2,police:4},belle_rive:{fire:2,samu:1,police:3},saint_roch:{fire:2,samu:1,police:3},valmont:{fire:1,samu:1,police:2},montfleur:{fire:2,samu:2,police:3}
};
// Numéro local de test volontairement non affiché dans l'interface publique.
const PRIVATE_FIRE_CHEAT_HASH=3300384764; // numéro privé non affiché dans l'interface
...
const FOOD_SHOP_TYPES=new Set(['corner']);

// V22.7 — santé, prescriptions, secours et restauration réaliste.
const DISEASES={
 foodPoisoning:{id:'foodPoisoning',name:'Intoxication alimentaire',icon:'🤢',medication:'gastroCare',doseHours:8,durationDays:3,doctorCost:18,hospitalCost:46,
  symptoms:['nausées','douleurs abdominales','fatigue','fièvre légère']}
};
const MEDICINES={
 gastroCare:{id:'gastroCare',name:'Traitement gastro-intestinal',icon:'💊',pricePerDose:2,desc:'Traitement symptomatique prescrit après consultation'}
};
const COUNTER_FOOD_TYPES=new Set(['bakery','butcher']);
const RESTAURANT_MENU=[
 {id:'meal',name:'Plat du jour',icon:'🍲',price:12,minutes:35},
 {id:'dinner',name:'Menu complet',icon:'🍽️',price:19,minutes:50},
 {id:'dessert',name:'Dessert maison',icon:'🍰',price:7,minutes:18}
];
function diseaseDef(){return state.disease?DISEASES[state.disease.id]:null}
function diseaseLabel(){if(!state.disease)return'En bonne santé';const d=diseaseDef();return`${d?.icon||'🤒'} ${d?.name||'Maladie'} • ${Math.round(state.disease.severity||0)}%`}
function diseaseStage(sev=state.disease?.severity||0){return sev>=78?'grave':sev>=50?'modérée':sev>=25?'légère':'faible'}
function currentPrescription(){return(state.prescriptions||[]).filter(x=>x.active!==false&&x.diseaseId===state.disease?.id).sort((a,b)=>b.issuedAt-a.issuedAt)[0]||null}
function activeMedicineCount(id){return Number(state.medications?.[id]||0)}
function createPrescription(source='doctor'){
 const d=diseaseDef();if(!d)return null;const total=Math.max(1,Math.ceil(24/d.doseHours*d.durationDays));
 state.prescriptions=state.prescriptions||[];for(const old of state.prescriptions)if(old.active!==false&&old.diseaseId===d.id)old.active=false;
 const rx={uid:`rx-${Date.now()}-${Math.random().toString(36).slice(2)}`,diseaseId:d.id,medicineId:d.medication,doseEveryHours:d.doseHours,durationDays:d.durationDays,totalDoses:total,remainingDoses:total,nextDoseAt:gameStamp(),issuedAt:gameStamp(),location:'bag',source,active:true,doctor:source==='hospital'?'Médecin hospitalier':'Médecin généraliste'};
 state.prescriptions.push(rx);return rx
}
function prescriptionDueText(rx){if(!rx)return'';const hrs=(rx.nextDoseAt-gameStamp())*24;if(hrs<=.05)return'prochaine prise : maintenant';return`prochaine prise : dans ~${Math.max(1,Math.ceil(hrs))} h`}
function prescriptionHTML(rx,home=false){const m=MEDICINES[rx.medicineId],loc=rx.location==='home'?'rangée au domicile':'dans le sac';return`<div class="card prescriptionCard"><div class="sectionKicker">ORDONNANCE</div><h3>📄 ${diseaseDef()?.name||DISEASES[rx.diseaseId]?.name||'Traitement'}</h3><p class="sub">${rx.doctor} • ${m?.name||rx.medicineId}<br><b>Posologie :</b> 1 prise toutes les ${rx.doseEveryHours} h pendant ${rx.durationDays} jours • ${rx.remainingDoses} prise(s) restante(s).<br>${prescriptionDueText(rx)} • ${loc}</p>${home?`<button class="tinyBtn movePrescriptionBag" data-id="${rx.uid}">Mettre dans le sac</button>`:`${atOwnResidence()?`<button class="tinyBtn movePrescriptionHome" data-id="${rx.uid}">Ranger chez soi</button>`:''}`}</div>`}
...
const base={
 ...
 landOwned:false,housingStage:0,homeLevel:1,homeBank:0,homeStorage:{medkit:0},homeStock:[],homePlaced:[],reputation:0,restCount:0,artifactBag:[],discoveredShops:[],hunger:70,thirst:70,hygiene:60,worldLayoutVersion:2291,trainTrips:0,visitedCities:['paris'],
 gameDay:1,gameMonth:1,agendaCustom:[],knownNpcOccupations:[],soundEnabled:true,avatarVersion:1,propertyCatalog:[],propertyPortfolio:[],residenceId:null,propertyCredit:0,monthlyLedger:'',missedRent:0,education:{current:null,completed:[]},job:null,workMission:null,companies:freshCompanies(),cityEconomies:freshCityEconomies(),cityTreasury:4800,taxPaid:0,salaryHistory:[],workCompleted:0,schoolDays:0,studyHours:0,avatar:{...AVATAR_DEFAULT},avatarCreated:false,cosmeticsUnlocked:[],workUniforms:{},uniformWorn:null,foodLots:[],foodStorage:{pantry:[],fridge:[],freezer:[]},shopCart:{shopType:null,items:[]},residenceFurniture:{},dentalHygiene:70,disease:null,prescriptions:[],medications:{},medicalBills:0,medicalDebt:0,hospitalFlow:null,phoneContacts:[]
};
let state=loadState();
...
function loadState(){
 try{
   ...
   // Interior scenes are not persisted; never keep a phantom hospital admission after a reload.
   loaded.hospitalFlow=null;
   ...
   loaded.worldLayoutVersion=2291;loaded.trainTrips=raw.trainTrips||0;loaded.visitedCities=raw.visitedCities||[loaded.cityId||'paris'];
...
function invCount(){const rx=(state.prescriptions||[]).filter(x=>x.location!=='home').length,med=Object.values(state.medications||{}).filter(q=>q>0).length;return state.inventory.reduce((a,x)=>a+x.qty,0)+(state.foodLots||[]).filter(x=>x.location==='bag').reduce((a,x)=>a+(x.qty||1),0)+Object.values(state.workUniforms||{}).filter(x=>x.location==='bag').length+rx+med}
...
let activeEnemy=null,activeEnemyEntity=null,moveStick={x:0,y:0},lookStick={x:0,y:0},weaponRig=null,interiorGroup=null,interiorSeller=null,interiorAmbientPeople=[],interiorHomeZones=[],interiorProductShelves=[],lastChunkTick=0,lastMapTick=0,lastHudTick=0,lastWeatherTick=0,lastShadowChunkKey='',selectedNPC=null,targetMarker=null,tailTheft=null,policeSeeing=false,hiddenTimer=0,lastCarHit=0,rainSystem=null,raycaster=null,tapStart=null,currentInteractFn=null,lastViewportHeight=window.innerHeight,keys={},lastPromptSig='',lastToastMessage='',lastToastAt=0,playerTrail=[],selectedProperty=null,bigMapZoom=.42,mapCenterOverride=null,mapFocusPropertyId=null,mapBusMode=false,mapWorldMode=false,currentBusStopId=null,currentBoardBusId=null,busWaitRequest=null,busRide=null,busJourneyPlan=null,busTicketValidUntil=0,busLastArrivalToast='',busLastUiTick=0,busCameraMode='window',busViewYaw=0,busViewPitch=-.04,interiorBounds={x:8.5,z:8.5},mpSocket=null,remotePlayers=new Map(),mpLastSend=0,mpLastX=0,mpLastZ=0,mpLastYaw=0,mpLastProfileSync=0,mpStatusMessage='Hors ligne',mpRoomCount=0,currentPanel=null,conversationNPC=null,emergencyResponse=null,samuResponse=null,policeResponse=null,policePatrols=[],servicePools=null,serviceDispatchQueue=[],lastServiceQueueTick=0,mapPhoneTarget=null,restaurantSession=null,lastDiseaseTick=0,selectedRemotePlayerId=null,voiceEnabled=false,localVoiceStream=null,voicePeers=new Map(),mutedPlayers=new Set(),uiAudioCtx=null;
...
async function init(){
 ...
 try{ensureChunks(true);ensureOutdoorPositionClear();initCityEmergencyServices()}
...
function busTrafficFactor(b,seg){
 ...
 for(const u of allServiceVehicleUnits()){const us=u.currentSeg||u.parkedSeg;if(!us||us.mode!==mode||us.dir!==dir||!u.vehicle?.parent)continue;const laneGap=mode==='h'?Math.abs(u.vehicle.position.z-b.group.position.z):Math.abs(u.vehicle.position.x-b.group.position.x);if(laneGap>1.3)continue;const delta=mode==='h'?(u.vehicle.position.x-b.group.position.x)*dir:(u.vehicle.position.z-b.group.position.z)*dir;if(delta>0&&delta<best)best=delta}
...
function busIntersectionState(b,seg,t){
 ...
 if(approaching){
   ...
   if(clear)for(const u of allServiceVehicleUnits()){const us=u.currentSeg;if(!us||us.mode===mode||!u.vehicle?.parent)continue;if(u.vehicle.position.x>ix-1.2&&u.vehicle.position.x<ix+12.2&&u.vehicle.position.z>iz-1.2&&u.vehicle.position.z<iz+12.2){clear=false;break}}
 }
...
function blockedByPerson(x,z){
 ...
 for(const n of allServiceResponders())if(Math.hypot(x-n.group.position.x,z-n.group.position.z)<.82)return true;
 for(const u of allServiceVehicleUnits())if(u.vehicle?.parent&&vehicleOccupiesPoint(u.vehicle,x,z,.22))return true;
 return false
}
...
function personDynamicBlocked(n,x,z,pad=.28){
 ...
 for(const r of allServiceResponders())if(r!==n&&Math.hypot(x-r.group.position.x,z-r.group.position.z)<Math.max(.92,pad+.56))return true;
 for(const u of allServiceVehicleUnits())if(u.vehicle?.parent&&vehicleOccupiesPoint(u.vehicle,x,z,.42))return true;
 return false
}
...
function walkCityTransition(link,exitSide,offset=0){
 const dest=CITIES.find(c=>c.id===link.to);if(!dest)return false;clearCityEmergencyServices();clearTarget(false);busWaitRequest=null;busJourneyPlan=null;busRide=null;currentBoardBusId=null;
 ...
 for(const[k]of[...chunks])unload(k);applyCityAtmosphere();initBusFleet();refreshCityLandmark();ensureChunks(true);initCityEmergencyServices();save();closeSheet();for(const rid of [...remotePlayers.keys()])removeRemoteAvatar(rid);
...
function updateCamera(t=0){const loading=emergencyResponse?.phase==='loading'&&emergencyResponse?.stretcher,rescue=emergencyResponse?.patientOnBoard&&emergencyResponse.vehicle,riding=busRide&&busVehicles.find(x=>x.id===busRide.vehicleId),bob=!riding&&!rescue&&!loading&&(Math.abs(moveStick.x)+Math.abs(moveStick.y)>.15)?Math.sin(t*.012)*.022:0;if(loading){const st=emergencyResponse.stretcher;camera.position.set(st.position.x,1.15,st.position.z+.15);const v=emergencyResponse.vehicle,dx=v.position.x-st.position.x,dz=v.position.z-st.position.z,d=Math.hypot(dx,dz)||1;camera.lookAt(st.position.x+dx/d*2,1.28,st.position.z+dz/d*2);return}if(rescue){rescue.updateMatrixWorld(true);const local=rescue.localToWorld(new THREE.Vector3(.32,1.48,.55));camera.position.copy(local);const a=-rescue.rotation.y,c=Math.cos(a),si=Math.sin(a);camera.lookAt(local.x+si,local.y-.04,local.z-c);return}if(riding){riding.group.updateMatrixWorld(true);const a=riding.group.rotation.y,baseYaw=-a;if(busCameraMode==='chase'){const yaw=baseYaw+busViewYaw,fx=Math.sin(yaw),fz=-Math.cos(yaw),cx=riding.group.position.x,cz=riding.group.position.z;camera.position.set(cx-fx*7.4,3.8+busViewPitch*1.8,cz-fz*7.4);camera.lookAt(cx,1.15,cz);return}const len=riding.group.userData.vehicleLength||5.6,local=busCameraMode==='front'?new THREE.Vector3(.22,1.70,-len/2-.16):new THREE.Vector3(.78,1.72,-Math.min(1.25,len*.24));const world=riding.group.localToWorld(local);camera.position.copy(world);const yaw=baseYaw+busViewYaw,pitch=busViewPitch,cp=Math.cos(pitch),sp=Math.sin(pitch),sy=Math.sin(yaw),cy=Math.cos(yaw);camera.lookAt(world.x+sy*cp,world.y+sp,world.z-cy*cp);return}let px=state.pos.x,pz=state.pos.z,py=1.72+bob;camera.position.set(px,py,pz);const cp=Math.cos(state.pitch),sp=Math.sin(state.pitch),sy=Math.sin(state.yaw),cy=Math.cos(state.yaw);camera.lookAt(px+sy*cp,py+sp,pz-cy*cp)}
...
function intersectionClearFor(c,ix,iz){
 ...
 for(const u of allServiceVehicleUnits()){const seg=u.currentSeg||u.parkedSeg;if(!seg||!u.vehicle?.parent)continue;const x=u.vehicle.position.x,z=u.vehicle.position.z;if(x>ix-1.2&&x<ix+12.2&&z>iz-1.2&&z<iz+12.2&&seg.mode!==c.mode)return false}
 return true
}
function serviceObstacleForCar(c,max=13){let best=null,bestD=max;for(const u of allServiceVehicleUnits()){if(!u.vehicle?.parent||!u.vehicle.userData.serviceParked)continue;const dx=u.vehicle.position.x-c.group.position.x,dz=u.vehicle.position.z-c.group.position.z;const d=c.mode==='v'?dz*c.dir:dx*c.dir,lateral=c.mode==='v'?Math.abs(dx):Math.abs(dz);if(d>0&&d<bestD&&lateral<2.3){best=u;bestD=d}}return best?{unit:best,d:bestD}:null}
function oppositeLaneFreeForCar(c){const px=c.group.position.x,pz=c.group.position.z,alt=c.mode==='v'?(c.dir>0?Math.floor(px/CHUNK)*CHUNK+7.8:Math.floor(px/CHUNK)*CHUNK+3.2):(c.dir>0?Math.floor(pz/CHUNK)*CHUNK+3.2:Math.floor(pz/CHUNK)*CHUNK+7.8);const occ=(x,z)=>{const lat=c.mode==='v'?Math.abs(x-alt):Math.abs(z-alt),lon=c.mode==='v'?Math.abs(z-pz):Math.abs(x-px);return lat<1.05&&lon<10};for(const o of cars)if(o!==c&&o.group?.parent&&occ(o.group.position.x,o.group.position.z))return false;for(const b of busVehicles)if(b.group?.parent&&occ(b.group.position.x,b.group.position.z))return false;for(const u of allServiceVehicleUnits())if(u.vehicle?.parent&&occ(u.vehicle.position.x,u.vehicle.position.z))return false;return true}
function tryMoveCar(c,dx,dz){const nx=c.group.position.x+dx,nz=c.group.position.z+dz;for(const u of allServiceVehicleUnits()){if(!u.vehicle?.parent)continue;if(vehicleOccupiesPoint(u.vehicle,nx,nz,.72))return false}c.group.position.x=nx;c.group.position.z=nz;return true}
function carAheadDistance(c,max=15){
 ...
 for(const u of allServiceVehicleUnits()){const seg=u.currentSeg;if(!seg||seg.mode!==c.mode||seg.dir!==c.dir||!u.vehicle?.parent)continue;const laneGap=c.mode==='v'?Math.abs(u.vehicle.position.x-c.group.position.x):Math.abs(u.vehicle.position.z-c.group.position.z);if(laneGap>1.15)continue;const delta=c.mode==='v'?(u.vehicle.position.z-c.group.position.z)*c.dir:(u.vehicle.position.x-c.group.position.x)*c.dir;if(delta>0&&delta<best)best=delta}
 return best
}
let lastTrafficMaintenance=0;
function recycleCarIntoChunk(c,g,key,cx,cz,seed=''){
 const r=rngFor(`traffic-recycle:${key}:${seed}:${Math.floor(performance.now()/4000)}`),x0=cx*CHUNK,z0=cz*CHUNK;
 const mode=r()<.5?'v':'h',dir=r()<.5?-1:1,along=18+r()*42;
 c.mode=mode;c.dir=dir;c.currentSpeed=Math.max(.8,c.speed*.55);c.stuckFor=0;
 c.group.position.set(mode==='v'?(dir>0?x0+3.2:x0+7.8):x0+along,0,mode==='v'?z0+along:(dir>0?z0+7.8:z0+3.2));
 c.group.rotation.y=mode==='v'?(dir>0?Math.PI:0):(dir>0?-Math.PI/2:Math.PI/2);
 if(c.group.parent!==g)g.add(c.group);c.key=key;
}
function maintainTrafficPopulation(t){
 if(t-lastTrafficMaintenance<2500||state.interior)return;lastTrafficMaintenance=t;
 const cc=currentChunk();
 for(const [key,g] of chunks){
   if(Math.abs(g.userData.cx-cc.cx)>1||Math.abs(g.userData.cz-cc.cz)>1)continue;
   if(cityChunkZone(g.userData.cx,g.userData.cz)!=='urban')continue;
   let local=cars.filter(c=>c.key===key&&c.group?.parent===g);
   // Cars that have driven far outside the active chunk are recycled rather
   // than disappearing forever, which was emptying streets after a few minutes.
   for(const c of local){const x0=g.userData.cx*CHUNK,z0=g.userData.cz*CHUNK;if(c.group.position.x<x0-18||c.group.position.x>x0+CHUNK+18||c.group.position.z<z0-18||c.group.position.z>z0+CHUNK+18)recycleCarIntoChunk(c,g,key,g.userData.cx,g.userData.cz,'out')}
   local=cars.filter(c=>c.key===key&&c.group?.parent===g);
   const minCars=3;
   for(let i=local.length;i<minCars;i++)addCar(g,key,g.userData.cx*CHUNK,g.userData.cz*CHUNK,rngFor(`traffic-maintain:${key}:${Math.floor(t/5000)}:${i}`),40+i);
 }
}
function updateCars(dt,t){
 ...
 for(const c of cars){
   ...
   const ahead=carAheadDistance(c,15),obstacle=serviceObstacleForCar(c,13),canPass=!!(obstacle&&obstacle.d<11&&oppositeLaneFreeForCar(c));
   const trafficFactor=obstacle&&obstacle.d<5.2&&!canPass?0:ahead<4.8?0:ahead<8.0?(ahead-4.8)/3.2:1;
   const inter=intersectionForCar(c,x0,z0),clear=intersectionClearFor(c,inter.x,inter.z);
   let distance=999,green=false;
   if(c.mode==='v'){
     const normalLane=c.dir>0?x0+3.2:x0+7.8,passLane=c.dir>0?x0+7.8:x0+3.2,laneX=canPass?passLane:normalLane;c.group.position.x+=(laneX-c.group.position.x)*Math.min(1,dt*(canPass?4.2:8));
     ...
     const normalLane=c.dir>0?z0+7.8:z0+3.2,passLane=c.dir>0?z0+3.2:z0+7.8,laneZ=canPass?passLane:normalLane;c.group.position.z+=(laneZ-c.group.position.z)*Math.min(1,dt*(canPass?4.2:8));
...
function updatePeople(dt,t){
 ...
 for(const n of npcs){
   ...
   else{const routine=updateNpcDailyRoutine(n,dt);if(routine==='patrol')patrolPerson(n,dt);else if(routine==='idle'){n.outdoorIdle=(n.outdoorIdle||0)+dt;if(n.outdoorIdle>1.8){patrolPerson(n,dt*.55);walking=true}else walking=false}else{n.outdoorIdle=0;walking=true}}
 ...
 for(const n of enemies){
   const dx=state.pos.x-n.group.position.x,dz=state.pos.z-n.group.position.z,dist=Math.hypot(dx,dz);
   const enemyVisible=dist<62||n===activeEnemyEntity||n.policeFleeUntil>t;n.group.visible=enemyVisible;if(!enemyVisible)continue;
   ensurePersonWalkable(n);if(n===activeEnemyEntity&&!(n.policeFleeUntil>t))continue;
   if(n.policeFleeUntil>t&&n.policeFleeFrom){const ax=n.group.position.x-n.policeFleeFrom.x,az=n.group.position.z-n.policeFleeFrom.z,ad=Math.hypot(ax,az)||1,sx=ax/ad*n.speed*1.85*dt,sz=az/ad*n.speed*1.85*dt;if(moveEntity(n,sx,sz,.3))setHeading(n,sx,sz);if(activeEnemyEntity===n&&ad>5){activeEnemyEntity=null;activeEnemy=null;$('#combat')?.classList.add('hidden');clearTarget(false);toast('👮 La police fait fuir ton agresseur.')}}
   else if(dist<8){const sx=dx/(dist||1)*n.speed*1.18*dt,sz=dz/(dist||1)*n.speed*1.18*dt;if(moveEntity(n,sx,sz,.3))setHeading(n,sx,sz);if(dist<1.5&&!activeEnemy)startCombat(n)}else patrolPerson(n,dt);
...
function civicPoisForCity(cityId=state.cityId){const net=TRANSIT_NETWORKS[cityId]||TRANSIT_NETWORKS.paris;return[...(net.pois||[]),...(SOCIETY_POIS[cityId]||[])]}
function nearestCivic(type,from=state.pos){return CIVIC_POIS.filter(p=>p.type===type).map(p=>({...p,d:Math.hypot(from.x-p.x,from.z-p.z)})).sort((a,b)=>a.d-b.d)[0]||null}
function nearestHospital(from=state.pos){const clinics=CIVIC_POIS.filter(p=>p.type==='clinic'),real=clinics.filter(p=>/hôpital|clinique/i.test(p.name||'')),pool=real.length?real:clinics;return pool.map(p=>({...p,d:Math.hypot(from.x-p.x,from.z-p.z)})).sort((a,b)=>a.d-b.d)[0]||null}
function nearestRoadPoint(p){const vx=Math.round((p.x-5.5)/CHUNK)*CHUNK+5.5,hz=Math.round((p.z-5.5)/CHUNK)*CHUNK+5.5;return Math.abs(p.x-vx)<Math.abs(p.z-hz)?{x:vx,z:p.z,road:true}:{x:p.x,z:hz,road:true}}
function emergencyRoute(a,b){
 const sa=nearestRoadPoint(a),sb=b?.road?{...b}:nearestRoadPoint(b),vx=Math.round((sa.x-5.5)/CHUNK)*CHUNK+5.5,hz=Math.round((sb.z-5.5)/CHUNK)*CHUNK+5.5;
 const finalPoint=b?.road?{x:b.x,z:b.z,road:true}:{x:b.x,z:b.z,offRoad:true};
 const pts=[{x:a.x,z:a.z,offRoad:true},{x:sa.x,z:sa.z,road:true},{x:vx,z:sa.z,road:true},{x:vx,z:hz,road:true},{x:sb.x,z:sb.z,road:true},finalPoint];
 return pts.filter((p,i)=>!i||Math.hypot(p.x-pts[i-1].x,p.z-pts[i-1].z)>.3)
}
function routeLength(pts){let d=0;for(let i=1;i<pts.length;i++)d+=Math.hypot(pts[i].x-pts[i-1].x,pts[i].z-pts[i-1].z);return d}
function emergencyParkingPoint(target,type='fire'){
 // V22.9.3: emergency units no longer stop in the middle of the nearest
 // intersection. They prefer a curb/sidewalk shoulder close to the caller,
 // at least ~16 m away from a junction, so traffic can still pass.
 const bx=Math.floor(target.x/CHUNK)*CHUNK,bz=Math.floor(target.z/CHUNK)*CHUNK;
 const clampAlong=(v,base)=>clamp(v,base+17,base+CHUNK-17),c=[];
 const add=(x,z,mode,dir,side)=>{
   const pt={x,z,mode,dir,road:false,offRoad:true,shoulder:true,side};
   // Avoid placing the centre of a 6 m emergency vehicle inside a building,
   // tree or another already parked response unit.
   let blocked=false;
   for(const q of colliders){if(x>q.minX-.7&&x<q.maxX+.7&&z>q.minZ-3.4&&z<q.maxZ+3.4){blocked=true;break}}
   if(!blocked)for(const u of allServiceVehicleUnits()){if(u.vehicle?.parent&&vehicleOccupiesPoint(u.vehicle,x,z,1.5)){blocked=true;break}}
   if(!blocked)c.push(pt)
 };
 // Vertical streets: park just over the curb on either sidewalk.
 add(bx+12.35,clampAlong(target.z,bz),'v',1,'east-curb');
 add(bx+CHUNK-1.65,clampAlong(target.z,bz),'v',-1,'west-curb');
 // Horizontal streets.
 add(clampAlong(target.x,bx),bz+12.35,'h',1,'south-curb');
 add(clampAlong(target.x,bx),bz+CHUNK-1.65,'h',-1,'north-curb');
 // Also examine the neighbouring block shoulders because the victim can be
 // standing near a chunk edge.
 const nbx=Math.floor((target.x+CHUNK*.5)/CHUNK)*CHUNK,nbz=Math.floor((target.z+CHUNK*.5)/CHUNK)*CHUNK;
 if(nbx!==bx){add(nbx+12.35,clampAlong(target.z,bz),'v',1,'east-curb');add(nbx-1.65,clampAlong(target.z,bz),'v',-1,'west-curb')}
 if(nbz!==bz){add(clampAlong(target.x,bx),nbz+12.35,'h',1,'south-curb');add(clampAlong(target.x,bx),nbz-1.65,'h',-1,'north-curb')}
 if(!c.length){const r=nearestRoadPoint(target);return{...r,mode:Math.abs(r.x-target.x)<Math.abs(r.z-target.z)?'v':'h',dir:1,road:true,shoulder:false}}
 c.sort((a,b)=>Math.hypot(a.x-target.x,a.z-target.z)-Math.hypot(b.x-target.x,b.z-target.z));
 return c[0]
}
function vehicleWorldPoint(vehicle,x,z,y=0){vehicle.updateMatrixWorld(true);return vehicle.localToWorld(new THREE.Vector3(x,y,z))}
function vehicleOccupiesPoint(vehicle,x,z,pad=.35){if(!vehicle?.parent)return false;const dx=x-vehicle.position.x,dz=z-vehicle.position.z,y=-vehicle.rotation.y,c=Math.cos(y),si=Math.sin(y),lx=dx*c-dz*si,lz=dx*si+dz*c,w=vehicle.userData.vehicleWidth||2.05,l=vehicle.userData.vehicleLength||4.5;return Math.abs(lx)<w/2+pad&&Math.abs(lz)<l/2+pad}
function inferParkedSeg(stop){return stop?.mode&&!stop?.shoulder?{mode:stop.mode,dir:stop.dir||1,laneCoord:stop.mode==='h'?stop.z:stop.x,roadBase:stop.mode==='h'?Math.floor(stop.z/CHUNK)*CHUNK:Math.floor(stop.x/CHUNK)*CHUNK,offRoad:false}:null}
function serviceVehicleHeadingForStop(stop,current=0){if(!stop?.mode)return current;return stop.mode==='h'?(stop.dir>0?-Math.PI/2:Math.PI/2):(stop.dir>0?Math.PI:0)}
function separateTrafficFromServiceVehicle(unit){
 if(!unit?.vehicle)return;const v=unit.vehicle;
 // A car that was already under the emergency vehicle when it parked is
 // moved a few metres forward on its own lane. This prevents the visual
 // interpenetration seen on iPhone while preserving continuous traffic.
 for(const c of cars){if(!c?.group?.parent||!vehicleOccupiesPoint(v,c.group.position.x,c.group.position.z,.9))continue;const push=5.8+(v.userData.vehicleLength||5.5)*.45;if(c.mode==='v')c.group.position.z+=c.dir*push;else c.group.position.x+=c.dir*push;c.currentSpeed=Math.min(c.currentSpeed||0,c.speed*.25);c.stuckFor=0}
 for(const b of busVehicles){if(!b?.group?.parent||!vehicleOccupiesPoint(v,b.group.position.x,b.group.position.z,1.0))continue;const seg=busSegment(b);if(seg){const push=6.8;if(seg.mode==='v')b.group.position.z+=seg.dir*push;else b.group.position.x+=seg.dir*push}}
}
function parkServiceUnit(unit){unit.currentSpeed=0;unit.currentSeg=null;unit.vehicle.rotation.y=serviceVehicleHeadingForStop(unit.stopPoint,unit.vehicle.rotation.y);unit.parkedSeg=inferParkedSeg(unit.stopPoint);unit.vehicle.userData.serviceParked=true;unit.vehicle.userData.serviceType=unit.type;unit.vehicle.userData.shoulderParked=!!unit.stopPoint?.shoulder;separateTrafficFromServiceVehicle(unit)}
function makeEmergencyUtilityBase(color=0xffffff){
 const g=new THREE.Group(),body=new THREE.MeshStandardMaterial({color,roughness:.58,metalness:.06}),dark=new THREE.MeshStandardMaterial({color:0x20262b,roughness:.48}),glass=new THREE.MeshStandardMaterial({color:0x29495c,roughness:.18,metalness:.08}),roofMat=new THREE.MeshStandardMaterial({color:0xf0f0ec,roughness:.72});
 // Proper long-wheelbase emergency van: low chassis, sloped visual cab and a
 // taller medical cell. The proportions are intentionally closer to a VSAV
 // than to the old stretched passenger car.
 const chassis=new THREE.Mesh(new THREE.BoxGeometry(2.28,.78,6.25),body);chassis.position.y=.64;g.add(chassis);
 const medical=new THREE.Mesh(new THREE.BoxGeometry(2.22,1.82,3.70),body);medical.position.set(0,1.70,.72);g.add(medical);
 const roof=new THREE.Mesh(new THREE.BoxGeometry(2.18,.16,3.74),roofMat);roof.position.set(0,2.64,.72);g.add(roof);
 const cab=new THREE.Mesh(new THREE.BoxGeometry(2.16,1.42,2.05),body);cab.position.set(0,1.28,-2.02);g.add(cab);
 const bonnet=new THREE.Mesh(new THREE.BoxGeometry(2.12,.58,.72),body);bonnet.position.set(0,.98,-3.06);g.add(bonnet);
 const wind=new THREE.Mesh(new THREE.BoxGeometry(1.76,.72,.07),glass);wind.position.set(0,1.58,-3.08);wind.rotation.x=-.08;g.add(wind);
 for(const sx of [-1,1]){const sideWin=new THREE.Mesh(new THREE.BoxGeometry(.07,.66,1.04),glass);sideWin.position.set(sx*1.09,1.54,-2.18);g.add(sideWin)}
 for(const sx of [-1,1])for(const z of [-2.20,1.78]){const wh=new THREE.Mesh(new THREE.CylinderGeometry(.46,.46,.24,14),dark);wh.rotation.z=Math.PI/2;wh.position.set(sx*1.16,.46,z);g.add(wh)}
 const bumperF=new THREE.Mesh(new THREE.BoxGeometry(2.18,.22,.18),dark);bumperF.position.set(0,.46,-3.43);g.add(bumperF);const bumperR=bumperF.clone();bumperR.position.z=3.16;g.add(bumperR);
 g.userData.vehicleLength=6.55;g.userData.vehicleWidth=2.34;g.userData.vehicleHeight=2.72;return g
}
function makeStretcher(){const g=new THREE.Group(),metal=new THREE.MeshStandardMaterial({color:0xd9dde0,metalness:.55,roughness:.35}),pad=new THREE.MeshStandardMaterial({color:0xe7a62d,roughness:.75}),wheel=new THREE.MeshStandardMaterial({color:0x222426,roughness:.72});const bed=new THREE.Mesh(new THREE.BoxGeometry(.72,.12,1.90),pad);bed.position.y=.72;g.add(bed);for(const x of [-.30,.30])for(const z of [-.72,.72]){const leg=new THREE.Mesh(new THREE.BoxGeometry(.06,.55,.06),metal);leg.position.set(x,.44,z);g.add(leg);const w=new THREE.Mesh(new THREE.CylinderGeometry(.10,.10,.06,10),wheel);w.rotation.z=Math.PI/2;w.position.set(x,.14,z);g.add(w)}g.userData.vehicleLength=1.9;g.userData.vehicleWidth=.72;return g}
function spawnCrewOutsideVehicle(vehicle,roles){const length=vehicle.userData.vehicleLength||5.4,width=vehicle.userData.vehicleWidth||2.2,spots=[[-width*.70,length*.18],[width*.70,length*.18],[-width*.72,-length*.18],[width*.72,-length*.18],[0,length*.62]];return roles.map((r,i)=>{const p=vehicleWorldPoint(vehicle,spots[i][0],spots[i][1]);const n=spawnResponder(r.role,p.x,p.z,r.label);n.group.rotation.y=vehicle.rotation.y;return n})}
function spawnCrewForDeployment(vehicle,roles){
 const length=vehicle.userData.vehicleLength||6.2,width=vehicle.userData.vehicleWidth||2.3;
 const door=[[-width*.52,.22],[width*.52,.22],[0,length*.42],[-width*.52,-1.15],[width*.52,-1.15]];
 const out=[[-width*1.05,.65],[width*1.05,.65],[-width*.95,length*.72],[width*.95,length*.60],[-width*1.10,-1.35]];
 return roles.map((r,i)=>{const p=vehicleWorldPoint(vehicle,door[i][0],door[i][1]),q=vehicleWorldPoint(vehicle,out[i][0],out[i][1]);const n=spawnResponder(r.role,p.x,p.z,r.label);n.group.rotation.y=vehicle.rotation.y;n.deployTarget={x:q.x,z:q.z};return n})
}
function updateCrewDeployment(list,dt){let all=true;for(const n of list||[]){const q=n.deployTarget;if(!q)continue;const dx=q.x-n.group.position.x,dz=q.z-n.group.position.z,d=Math.hypot(dx,dz);if(d>.12){all=false;const k=Math.min(d,1.65*dt);n.group.position.x+=dx/d*k;n.group.position.z+=dz/d*k;setHeading(n,dx,dz)}else delete n.deployTarget}return all}
function setResponderCarePose(n,amount=.65){if(!n)return;if(n.arms){n.arms[0].rotation.x=-1.0*amount;n.arms[1].rotation.x=-.82*amount}if(n.legs){n.legs[0].rotation.x=.20*amount;n.legs[1].rotation.x=-.08*amount}}
function animateCrewCare(list,target,dt,t){const form=[[-1.25,.15],[1.25,.15],[0,1.45],[-1.65,1.25],[1.65,1.25]];for(let i=0;i<list.length;i++){const n=list[i],o=form[i]||[0,1.2+i*.35],tx=target.x+o[0],tz=target.z+o[1],dx=tx-n.group.position.x,dz=tz-n.group.position.z,d=Math.hypot(dx,dz);if(d>.18){const k=Math.min(d,1.45*dt);n.group.position.x+=dx/d*k;n.group.position.z+=dz/d*k;setHeading(n,dx,dz)}else{setHeading(n,target.x-n.group.position.x,target.z-n.group.position.z);setResponderCarePose(n,.70+.12*Math.sin(t*.005+i))}}}
function ensureFireStretcher(e){if(e.stretcher?.parent)return e.stretcher;const st=makeStretcher(),p=vehicleWorldPoint(e.vehicle,0,(e.vehicle.userData.vehicleLength||5.4)*.58);st.position.set(p.x,0,p.z);st.rotation.y=e.vehicle.rotation.y;scene.add(st);e.stretcher=st;e.stretcherStart={x:p.x,z:p.z};return st}
function removeServiceStretcher(e){if(e?.stretcher?.parent)scene.remove(e.stretcher);if(e)e.stretcher=null}
function emergencyConfirmHTML(){const e=emergencyResponse;if(!e)return'<div class="card"><p class="sub">Aucune intervention en attente de confirmation.</p></div>';return `<div class="card"><div class="sectionKicker">SAPEURS-POMPIERS</div><h3>🚒 Le chef d’agrès te parle</h3><p class="sub">« Bonjour. Vous avez demandé les secours. Est-ce que vous confirmez que vous souhaitez être examiné(e) et pris(e) en charge ? »</p><p class="sub">Le véhicule reste stationné et l’équipe attend ta réponse.</p><button class="menuBtn primary fireCareAccept" style="width:100%">Oui, je confirme la prise en charge</button><button class="menuBtn fireCareDecline" style="width:100%;margin-top:8px">Non, je refuse l’intervention</button></div>`}
function acceptFireCare(){const e=emergencyResponse;if(!e||e.phase!=='confirm')return closeSheet();e.phase='assess';e.careActive=true;e.dwell=e.forceTest&&!state.disease?5.5:7.5;e.careElapsed=0;removeServiceStretcher(e);e.statusText=`Bilan en cours • environ ${Math.ceil(e.dwell)} s`;closeSheet();toast('🚒 « Très bien. On commence par le bilan. Le brancard ne sera sorti que si un transport est nécessaire. »')}
function declineFireCare(){const e=emergencyResponse;if(!e||e.phase!=='confirm')return closeSheet();removeResponderCrew(e.crew);removeServiceStretcher(e);e.careActive=false;e.phase='returning';e.emergency=false;e.patientOnBoard=false;e.stopPoint=nearestRoadPoint(e.station);e.route=emergencyRoute(e.vehicle.position,e.stopPoint);e.index=1;e.parkedSeg=null;e.vehicle.userData.serviceParked=false;e.statusText='Intervention refusée • retour à la caserne';closeSheet();toast('🚒 Les pompiers prennent acte de ton refus et repartent.')}
function beginFireLoading(e,urgent=false){e.transportUrgent=urgent;e.phase='loading';e.loadingElapsed=0;e.loadingDuration=6.5;e.loadingStart={x:state.pos.x,z:state.pos.z};e.careActive=true;e.hospital=nearestHospital(e.vehicle.position);if(!e.hospital){toast('Aucun hôpital disponible.');declineFireCare();return}ensureFireStretcher(e);e.statusText=`Installation sur le brancard • ~${Math.ceil(e.loadingDuration)} s`;toast('🚑 Le brancard est sorti du VSAV. Deux pompiers t’installent avant le transport.')}
function animateFireLoading(e,dt,t){const st=ensureFireStretcher(e);e.loadingElapsed=(e.loadingElapsed||0)+dt;const dur=e.loadingDuration||6.5,p=Math.min(1,e.loadingElapsed/dur),rear=vehicleWorldPoint(e.vehicle,0,(e.vehicle.userData.vehicleLength||6.4)*.58),start=e.loadingStart||{x:state.pos.x,z:state.pos.z};
 // First half: stretcher approaches patient. Second half: patient + stretcher
 // are rolled back to the rear doors, with responders visibly surrounding it.
 let sx,sz;if(p<.46){const q=p/.46;sx=rear.x+(start.x-rear.x)*q;sz=rear.z+(start.z-rear.z)*q}else{const q=(p-.46)/.54;sx=start.x+(rear.x-start.x)*q;sz=start.z+(rear.z-start.z)*q;state.pos.x=sx;state.pos.z=sz}
 st.position.x=sx;st.position.z=sz;st.rotation.y=e.vehicle.rotation.y;animateCrewCare(e.crew,{x:sx,z:sz},dt,t);e.statusText=`Installation sur le brancard • ~${Math.max(0,Math.ceil(dur-e.loadingElapsed))} s`;
 if(p<1)return false;removeResponderCrew(e.crew);removeServiceStretcher(e);e.patientOnBoard=true;e.careActive=false;e.phase='toHospital';e.emergency=!!e.transportUrgent;e.stopPoint=nearestRoadPoint(e.hospital);e.route=emergencyRoute(e.vehicle.position,e.stopPoint);e.index=1;e.parkedSeg=null;e.vehicle.userData.serviceParked=false;e.statusText=`Transport ${e.emergency?'urgent ':''}vers ${e.hospital.name}`;toast(`🚑 Installation terminée. Départ ${e.emergency?'en urgence ':''}vers ${e.hospital.name}.`);return true}
function makeEmergencyLightBar(g,y=1.82){const redMat=new THREE.MeshBasicMaterial({color:0xff3344}),blueMat=new THREE.MeshBasicMaterial({color:0x3f9dff});const red=new THREE.Mesh(new THREE.BoxGeometry(.48,.12,.30),redMat),blue=new THREE.Mesh(new THREE.BoxGeometry(.48,.12,.30),blueMat);red.position.set(-.25,y,-.1);blue.position.set(.25,y,-.1);red.visible=blue.visible=false;g.add(red,blue);g.userData.sirenLights=[red,blue];return g}
function flashEmergencyLights(g,t,enabled=true){const a=g?.userData?.sirenLights;if(!a)return;if(!enabled){a[0].visible=a[1].visible=false;return}const flip=Math.floor(t/170)%2===0;a[0].visible=flip;a[1].visible=!flip}
function makeFireResponseVehicle(){
 const g=makeEmergencyUtilityBase(0xd6302f);makeEmergencyLightBar(g,2.82);
 const white=new THREE.MeshStandardMaterial({color:0xf4f3ed,roughness:.64}),yellow=new THREE.MeshStandardMaterial({color:0xf4d23c,roughness:.62});
 // Broad reflective belt and high white medical-cell panels.
 const belt=new THREE.Mesh(new THREE.BoxGeometry(2.37,.22,5.70),yellow);belt.position.set(0,1.05,.12);g.add(belt);
 const upper=new THREE.Mesh(new THREE.BoxGeometry(2.25,.86,3.55),white);upper.position.set(0,2.12,.72);g.add(upper);
 for(const sx of [-1,1]){const doorLine=new THREE.Mesh(new THREE.BoxGeometry(.035,1.55,1.52),new THREE.MeshStandardMaterial({color:0xb51f25,roughness:.7}));doorLine.position.set(sx*1.135,1.68,1.25);g.add(doorLine)}
 const front=makeSign('SAPEURS-POMPIERS','#ffffff');front.scale.set(.94,.22,.24);front.position.set(0,1.98,-3.46);front.rotation.y=Math.PI;g.add(front);
 const rear=makeSign('VSAV • 18','#ffffff');rear.scale.set(.82,.22,.24);rear.position.set(0,1.78,3.22);g.add(rear);
 g.userData.vehicleLength=6.55;g.userData.vehicleWidth=2.34;g.userData.vehicleHeight=2.82;return g
}
function makeRescueVehicle(){return makeFireResponseVehicle()}
function makeSamuVehicle(){const g=makeEmergencyUtilityBase(0xf0f1ef);makeEmergencyLightBar(g,2.62);const blue=new THREE.MeshStandardMaterial({color:0x285b8c,roughness:.55});for(const z of [-1.45,.10,1.55]){const stripe=new THREE.Mesh(new THREE.BoxGeometry(2.16,.17,.54),blue);stripe.position.set(0,.90,z);g.add(stripe)}const sign=makeSign('SAMU • SMUR','#164f82');sign.scale.set(1.18,.28,.28);sign.position.set(0,1.88,2.78);g.add(sign);const cross=makeSign('✚','#d93f45');cross.scale.set(.42,.26,.28);cross.position.set(.60,1.72,-2.84);cross.rotation.y=Math.PI;g.add(cross);return g}
function makePoliceResponseVehicle(){const g=createCarVisual(0x244d78,'sedan');g.userData.vehicleLength=4.55;g.userData.vehicleWidth=1.92;g.userData.vehicleHeight=1.52;makeEmergencyLightBar(g,1.56);const sign=makeSign('POLICE','#d8efff');sign.scale.set(1.15,.28,.3);sign.position.set(0,1.18,2.03);g.add(sign);return g}
function serviceCapacity(cityId=state.cityId){return EMERGENCY_SERVICE_CAPACITY[cityId]||EMERGENCY_SERVICE_CAPACITY.paris}
function ensureServicePools(){if(servicePools)return servicePools;servicePools={};for(const c of CITIES){const cap=serviceCapacity(c.id),r=rngFor(`service-load:${c.id}:${absoluteGameDay()}:${Math.floor(state.timeOfDay/4)}`),rf=r(),rs=r(),rp=r();const fireBg=rf<.10?cap.fire:rf<.42?Math.min(1,Math.max(0,cap.fire-1)):0,samuBg=rs<.14?cap.samu:rs<.44?Math.min(1,Math.max(0,cap.samu-1)):0,policeBg=rp<.28?Math.min(1,Math.max(0,cap.police-1)):0;servicePools[c.id]={fireActive:0,samuActive:0,policeActive:0,fireBackground:fireBg,samuBackground:samuBg,policeBackground:policeBg}}return servicePools}
function serviceAvailable(type,cityId=state.cityId){ensureServicePools();const cap=serviceCapacity(cityId)[type]||0,p=servicePools[cityId],active=p?.[type+'Active']||0,bg=p?.[type+'Background']||0;return Math.max(0,cap-active-bg)}
function reserveServiceToken(type,cityId){ensureServicePools();if(serviceAvailable(type,cityId)<=0)return null;const p=servicePools[cityId];p[type+'Active']=(p[type+'Active']||0)+1;return{type,cityId,id:`${type}:${cityId}:${Date.now()}:${Math.random().toString(36).slice(2)}`}}
function releaseServiceToken(token){if(!token||token.released)return;ensureServicePools();const p=servicePools[token.cityId];if(p)p[token.type+'Active']=Math.max(0,(p[token.type+'Active']||0)-1);token.released=true}
function cityRoadPath(from,to){const q=[[from]],seen=new Set([from]);while(q.length){const path=q.shift(),last=path[path.length-1];if(last===to)return path;for(const e of WALK_LINKS){const n=e.a===last?e.b:e.b===last?e.a:null;if(n&&!seen.has(n)){seen.add(n);q.push([...path,n])}}}return null}
function mutualAidEntrySide(originId,destId){const path=cityRoadPath(originId,destId);if(!path||path.length<2)return null;const prev=path[path.length-2];for(const e of WALK_LINKS){if(e.a===prev&&e.b===destId)return e.bSide;if(e.b===prev&&e.a===destId)return e.aSide}return null}
function reserveBestService(type){let token=reserveServiceToken(type,state.cityId);if(token)return{token,mutualAid:false,delay:0,originCity:state.cityId};const choices=CITIES.filter(c=>c.id!==state.cityId&&serviceAvailable(type,c.id)>0).map(c=>{const p=cityRoadPath(c.id,state.cityId);return{c,hops:p?p.length-1:99}}).filter(x=>x.hops<99).sort((a,b)=>a.hops-b.hops);if(!choices.length)return null;const pick=choices[0];token=reserveServiceToken(type,pick.c.id);return{token,mutualAid:true,originCity:pick.c.id,delay:Math.max(12,pick.hops*18),entrySide:mutualAidEntrySide(pick.c.id,state.cityId)}}
function reserveMutualAidService(type){const choices=CITIES.filter(c=>c.id!==state.cityId&&serviceAvailable(type,c.id)>0).map(c=>{const p=cityRoadPath(c.id,state.cityId);return{c,hops:p?p.length-1:99}}).filter(x=>x.hops<99).sort((a,b)=>a.hops-b.hops);if(!choices.length)return null;const pick=choices[0],token=reserveServiceToken(type,pick.c.id);return token?{token,mutualAid:true,originCity:pick.c.id,delay:Math.max(12,pick.hops*18),entrySide:mutualAidEntrySide(pick.c.id,state.cityId)}:null}
function serviceStatusLine(type){const label={fire:'Pompiers',samu:'SAMU',police:'Police'}[type]||type,cap=serviceCapacity()[type]||0,avail=serviceAvailable(type);return`${label} : ${avail}/${cap} équipe${cap>1?'s':''} disponible${avail>1?'s':''}`}
function spawnResponder(role,x,z,label=''){const policeRole=role==='police',n=createPerson(policeRole?'police':'civilian',`service:${role}:${Date.now()}:${Math.random()}`,x,z,Math.random);const wear=role==='firefighter'?JOB_WORKWEAR.firefighter:role==='doctor'?JOB_WORKWEAR.doctor:role==='nurse'?JOB_WORKWEAR.nurse:role==='ambulance'?JOB_WORKWEAR.careAssistant:null;if(wear&&!policeRole){n.clothMat.color.set(wear.top);n.pantsMat.color.set(wear.pants);n.casualTop=Number.parseInt(wear.top.replace('#',''),16);n.casualPants=Number.parseInt(wear.pants.replace('#',''),16)}n.occupation={...(n.occupation||{}),title:label||({firefighter:'Sapeur-pompier',doctor:'Médecin SAMU',nurse:'Infirmier SAMU',ambulance:'Ambulancier',police:'Policier'}[role]||'Secouriste')};n.serviceResponder=true;n.speed=.95;scene.add(n.group);return n}
function removeResponderCrew(list=[]){for(const n of list)if(n?.group?.parent)scene.remove(n.group);list.length=0}
function moveResponderCrewToward(list,target,dt,speed=1.35){const form=[[-1.15,.25],[1.15,.25],[0,1.35],[-1.55,1.20],[1.55,1.20]];for(let i=0;i<list.length;i++){const n=list[i],o=form[i]||[(i%2?1:-1)*1.25,1.1+Math.floor(i/2)*.55],tx=target.x+o[0],tz=target.z+o[1],dx=tx-n.group.position.x,dz=tz-n.group.position.z,d=Math.hypot(dx,dz);if(d>.18){const k=Math.min(d,speed*dt);n.group.position.x+=dx/d*k;n.group.position.z+=dz/d*k;setHeading(n,dx,dz)}}}
function allServiceVehicleUnits(){const out=[];if(emergencyResponse?.vehicle)out.push(emergencyResponse);if(samuResponse?.vehicle)out.push(samuResponse);for(const p of policePatrols)if(p?.vehicle)out.push(p);return out}
function allServiceResponders(){const out=[];for(const u of [emergencyResponse,samuResponse,...policePatrols])for(const n of (u?.crew||u?.officers||[]))if(n?.group?.parent)out.push(n);return out}
function serviceRoadSegment(unit){
 if(!unit?.vehicle||!unit.route?.length)return null;
 const raw=unit.route?.[unit.index];if(!raw)return null;
 const pos=unit.vehicle.position;
 // IMPORTANT V22.9.1: the old code recalculated the axis from the vehicle's
 // current position every frame. Near a corner this could flip H/V repeatedly,
 // making emergency vehicles oscillate forever. The segment axis is now derived
 // from the authored polyline and stays stable for the whole segment.
 const prev=unit.route?.[Math.max(0,unit.index-1)]||{x:pos.x,z:pos.z};
 const pdx=raw.x-prev.x,pdz=raw.z-prev.z;
 const offRoad=!!raw.offRoad||!!prev.offRoad;
 const mode=Math.abs(pdx)>=Math.abs(pdz)?'h':'v';
 const dir=mode==='h'?(pdx>=0?1:-1):(pdz>=0?1:-1);
 let tx=raw.x,tz=raw.z,laneCoord=null,roadBase=null;
 if(!offRoad){
   if(mode==='h'){
     roadBase=Math.round((raw.z-5.5)/CHUNK)*CHUNK;
     laneCoord=roadBase+(dir>0?7.8:3.2);tz=laneCoord;
   }else{
     roadBase=Math.round((raw.x-5.5)/CHUNK)*CHUNK;
     laneCoord=roadBase+(dir>0?3.2:7.8);tx=laneCoord;
   }
 }
 const dx=tx-pos.x,dz=tz-pos.z;
 return{target:{...raw,x:tx,z:tz},raw,dx,dz,dist:Math.hypot(dx,dz),mode,dir,laneCoord,roadBase,offRoad};
}
function serviceTrafficAhead(unit,seg,max=18){let best=max;const px=unit.vehicle.position.x,pz=unit.vehicle.position.z;const check=(x,z,mode,dir)=>{if(mode!==seg.mode||dir!==seg.dir)return;const laneGap=seg.mode==='h'?Math.abs(z-pz):Math.abs(x-px);if(laneGap>1.25)return;const delta=seg.mode==='h'?(x-px)*seg.dir:(z-pz)*seg.dir;if(delta>0&&delta<best)best=delta};for(const c of cars)if(c?.group?.parent)check(c.group.position.x,c.group.position.z,c.mode,c.dir);for(const b of busVehicles){const bs=busSegment(b);if(bs&&b.group?.parent)check(b.group.position.x,b.group.position.z,bs.mode,bs.dir)}for(const o of allServiceVehicleUnits()){if(o===unit||!o.vehicle?.parent||!(o.currentSeg||o.parkedSeg))continue;const os=o.currentSeg||o.parkedSeg;check(o.vehicle.position.x,o.vehicle.position.z,os.mode,os.dir)}return best}
function alternateLaneSafe(unit,seg){if(seg.offRoad)return false;const alt=seg.mode==='h'?seg.roadBase+(seg.dir>0?3.2:7.8):seg.roadBase+(seg.dir>0?7.8:3.2),px=unit.vehicle.position.x,pz=unit.vehicle.position.z;const occupied=(x,z)=>{const lateral=seg.mode==='h'?Math.abs(z-alt):Math.abs(x-alt),long=seg.mode==='h'?Math.abs(x-px):Math.abs(z-pz);return lateral<1.2&&long<13};for(const c of cars)if(c?.group?.parent&&occupied(c.group.position.x,c.group.position.z))return false;for(const b of busVehicles)if(b?.group?.parent&&occupied(b.group.position.x,b.group.position.z))return false;for(const o of allServiceVehicleUnits())if(o!==unit&&o.vehicle?.parent&&occupied(o.vehicle.position.x,o.vehicle.position.z))return false;return true}
function serviceIntersectionState(unit,seg,t,priority){if(seg.offRoad)return{mustStop:false,distance:99,crossSlow:false};const cx=Math.floor(unit.vehicle.position.x/CHUNK),cz=Math.floor(unit.vehicle.position.z/CHUNK),x0=cx*CHUNK,z0=cz*CHUNK,lights=trafficSignalState(t);let stopAt,distance,green,ix,iz;if(seg.mode==='v'){stopAt=seg.dir>0?z0+CHUNK-2.45:z0+13.15;distance=seg.dir>0?stopAt-unit.vehicle.position.z:unit.vehicle.position.z-stopAt;green=lights.vertical;ix=x0;iz=seg.dir>0?z0+CHUNK:z0}else{stopAt=seg.dir>0?x0+CHUNK-2.45:x0+13.15;distance=seg.dir>0?stopAt-unit.vehicle.position.x:unit.vehicle.position.x-stopAt;green=lights.horizontal;ix=seg.dir>0?x0+CHUNK:x0;iz=z0}const approaching=distance>=0&&distance<12;let clear=true;if(approaching){for(const c of cars){if(c?.group?.parent&&carInsideIntersection(c,ix,iz)&&c.mode!==seg.mode){clear=false;break}}if(clear)for(const b of busVehicles){const bs=busSegment(b);if(bs&&bs.mode!==seg.mode&&b.group.position.x>ix-1.2&&b.group.position.x<ix+12.2&&b.group.position.z>iz-1.2&&b.group.position.z<iz+12.2){clear=false;break}}if(clear)for(const o of allServiceVehicleUnits()){if(o===unit||!o.vehicle?.parent||!(o.currentSeg||o.parkedSeg)||(o.currentSeg||o.parkedSeg).mode===seg.mode)continue;if(o.vehicle.position.x>ix-1.2&&o.vehicle.position.x<ix+12.2&&o.vehicle.position.z>iz-1.2&&o.vehicle.position.z<iz+12.2){clear=false;break}}}if(!priority)return{mustStop:approaching&&(!green||!clear),distance,crossSlow:false};if(approaching&&!clear)return{mustStop:true,distance,crossSlow:false};return{mustStop:false,distance,crossSlow:approaching&&!green}}
function serviceDrive(unit,dt,t){
 if(!unit?.vehicle||!unit.route?.length)return false;
 if(unit.preDelay>0){unit.preDelay=Math.max(0,unit.preDelay-dt);flashEmergencyLights(unit.vehicle,t,false);unit.vehicle.visible=false;if(unit.preDelay<=0)unit.vehicle.visible=true;return false}
 unit.vehicle.visible=true;
 let seg=serviceRoadSegment(unit);unit.currentSeg=seg;if(!seg)return true;
 if(seg.dist<.45){unit.vehicle.position.set(seg.target.x,0,seg.target.z);unit.index++;unit._stuck=0;unit._lastProgressDist=Infinity;seg=serviceRoadSegment(unit);unit.currentSeg=seg;if(!seg)return true}
 const priority=!!unit.emergency;flashEmergencyLights(unit.vehicle,t,priority);
 const ahead=serviceTrafficAhead(unit,seg,20);
 let trafficFactor=ahead<5.4?0:ahead<10?(ahead-5.4)/4.6:1;
 let laneTarget=seg.laneCoord,overtaking=false;
 // Priority vehicles may use the opposite lane when it is demonstrably clear.
 if(priority&&ahead<10&&alternateLaneSafe(unit,seg)){
   overtaking=true;
   laneTarget=seg.mode==='h'?seg.roadBase+(seg.dir>0?3.2:7.8):seg.roadBase+(seg.dir>0?7.8:3.2);
   trafficFactor=Math.max(.72,trafficFactor);
 }
 const signal=serviceIntersectionState(unit,seg,t,priority);
 let targetSpeed=(unit.baseSpeed||7.5)*(seg.offRoad?.50:1)*trafficFactor;
 if(signal.mustStop)targetSpeed=0;
 if(signal.crossSlow)targetSpeed=Math.min(targetSpeed,2.3);
 const pd=Math.hypot(state.pos.x-unit.vehicle.position.x,state.pos.z-unit.vehicle.position.z);
 if(!unit.patientOnBoard&&pd<3.0)targetSpeed=Math.min(targetSpeed,1.45);
 // Watchdog: an emergency response must never remain frozen forever behind a
 // stale traffic object. It still slows at occupied intersections, but after a
 // few seconds without progress it searches for a safe crawl/overtake.
 const progressDist=seg.dist;
 if(!Number.isFinite(unit._lastProgressDist)||progressDist<unit._lastProgressDist-.08){unit._lastProgressDist=progressDist;unit._stuck=0}
 else unit._stuck=(unit._stuck||0)+dt;
 if(priority&&unit._stuck>2.8&&!signal.mustStop){targetSpeed=Math.max(targetSpeed,2.0);trafficFactor=Math.max(trafficFactor,.35)}
 if(priority&&unit._stuck>5.5&&alternateLaneSafe(unit,seg)){targetSpeed=Math.max(targetSpeed,3.0);overtaking=true;laneTarget=seg.mode==='h'?seg.roadBase+(seg.dir>0?3.2:7.8):seg.roadBase+(seg.dir>0?7.8:3.2)}
 if(unit._stuck>9){
   // Rebuild only the remaining physical route from the vehicle's CURRENT
   // position. No teleportation: the vehicle keeps driving from where it is.
   const destination=unit.stopPoint||unit.target||unit.route[unit.route.length-1];
   unit.route=emergencyRoute({x:unit.vehicle.position.x,z:unit.vehicle.position.z},destination);unit.index=1;unit._stuck=0;unit._lastProgressDist=Infinity;
   seg=serviceRoadSegment(unit);unit.currentSeg=seg;if(!seg)return true;
 }
 unit.currentSpeed=(unit.currentSpeed||0)+(targetSpeed-(unit.currentSpeed||0))*Math.min(1,dt*(targetSpeed<(unit.currentSpeed||0)?6.6:2.8));
 if(signal.mustStop&&signal.distance<.45)unit.currentSpeed=0;
 let dx=seg.dx,dz=seg.dz;
 if(!seg.offRoad&&laneTarget!=null){if(seg.mode==='h')dz=laneTarget-unit.vehicle.position.z;else dx=laneTarget-unit.vehicle.position.x}
 const targetYaw=Math.atan2(-dx,-dz),angle=Math.atan2(Math.sin(targetYaw-unit.vehicle.rotation.y),Math.cos(targetYaw-unit.vehicle.rotation.y)),yawRate=priority?1.55:1.05;
 unit.vehicle.rotation.y+=clamp(angle,-yawRate*dt,yawRate*dt);
 const headingError=Math.abs(angle),headFactor=headingError>.95?.22:headingError>.5?.55:1;
 let step=Math.min(seg.dist,Math.max(0,unit.currentSpeed)*headFactor*dt);
 if(signal.mustStop&&signal.distance>=0)step=Math.min(step,Math.max(0,signal.distance-.30));
 if(step>.001){const fx=-Math.sin(unit.vehicle.rotation.y),fz=-Math.cos(unit.vehicle.rotation.y);unit.vehicle.position.x+=fx*step;unit.vehicle.position.z+=fz*step;if(!seg.offRoad&&laneTarget!=null&&headingError<.22){const k=Math.min(1,dt*(overtaking?3.4:2.4));if(seg.mode==='h')unit.vehicle.position.z+=(laneTarget-unit.vehicle.position.z)*k;else unit.vehicle.position.x+=(laneTarget-unit.vehicle.position.x)*k}}
 return false;
}
function backgroundBusy(type,cityId){ensureServicePools();return servicePools[cityId]?.[type+'Background']||0}
function serviceMutualAidSpawn(res,type){const c=city(),side=res.entrySide||'west',pos=walkingEntryPosition(c,side,0),vehicle=type==='fire'?makeFireResponseVehicle():type==='samu'?makeSamuVehicle():makePoliceResponseVehicle();vehicle.position.set(pos.x,0,pos.z);vehicle.visible=res.delay<=0;scene.add(vehicle);return vehicle}
function localServiceSpawn(res,type){const pois=civicPoisForCity(state.cityId),stationType=type==='police'?'policeStation':type==='fire'?'fireStation':'clinic',station=type==='samu'?(pois.find(p=>p.type==='clinic'&&/hôpital|clinique/i.test(p.name||''))||pois.find(p=>p.type==='clinic')):(pois.find(p=>p.type===stationType)||nearestCivic(stationType));const vehicle=type==='fire'?makeFireResponseVehicle():type==='samu'?makeSamuVehicle():makePoliceResponseVehicle();const p=station||{x:0,z:0,name:'Base'};vehicle.position.set(p.x,0,p.z);scene.add(vehicle);return{vehicle,station:p}}
function createDispatchUnit(type,res,target,opts={}){let vehicle,station;if(res.mutualAid){vehicle=serviceMutualAidSpawn(res,type);station={name:`Renfort ${CITIES.find(c=>c.id===res.originCity)?.name||res.originCity}`,x:vehicle.position.x,z:vehicle.position.z}}else{const v=localServiceSpawn(res,type);vehicle=v.vehicle;station=v.station}const stop=emergencyParkingPoint(target,type),route=emergencyRoute(vehicle.position,stop);return{type,token:res.token,originCity:res.originCity,mutualAid:res.mutualAid,preDelay:res.delay||0,vehicle,station,route,index:1,baseSpeed:opts.speed||9,currentSpeed:0,emergency:opts.emergency!==false,target:{...target},stopPoint:stop,patientOnBoard:false,crew:[],statusText:'',phase:'toCall',dwell:0,parkedSeg:null,stretcher:null}}
function serviceEta(unit){return Math.max(5,Math.ceil((unit.preDelay||0)+routeLength(unit.route)/(unit.baseSpeed||8)))}
function refreshServiceStatus(unit,label){if(!unit?.vehicle||!unit.route?.length)return;const remaining=unit.route.slice(Math.max(0,unit.index-1));const eta=Math.max(1,Math.ceil(routeLength([{x:unit.vehicle.position.x,z:unit.vehicle.position.z},...remaining])/(unit.baseSpeed||8)+(unit.preDelay||0)));unit.liveEta=eta;if(unit.phase==='toPatient'||unit.phase==='toCall')unit.statusText=`${label} • ~${eta} s`; }
function dispatchQueued(type,cb){serviceDispatchQueue.push({type,cb,at:performance.now()});toast(`⏳ Toutes les équipes ${type==='fire'?'de pompiers':type==='samu'?'du SAMU':'de police'} sont engagées. Ton appel est placé en attente.`)}
function updateServiceQueues(t){if(t-lastServiceQueueTick<1000)return;lastServiceQueueTick=t;for(let i=serviceDispatchQueue.length-1;i>=0;i--){const q=serviceDispatchQueue[i];let res=q.type==='police'?reserveMutualAidService('police'):reserveBestService(q.type);if(!res&&t-q.at>15000){ensureServicePools();const nearest=[state.cityId,...CITIES.filter(c=>c.id!==state.cityId).map(c=>c.id)].find(id=>(servicePools[id]?.[q.type+'Background']||0)>0);if(nearest){servicePools[nearest][q.type+'Background']=Math.max(0,servicePools[nearest][q.type+'Background']-1);if(q.type==='police'){if(nearest===state.cityId){const token=reserveServiceToken('police',state.cityId);res=token?{token,mutualAid:false,delay:0,originCity:state.cityId}:null}else res=reserveMutualAidService('police')}else res=reserveBestService(q.type)}}if(res){serviceDispatchQueue.splice(i,1);q.cb(res)}}}
function callFirefighters(forceTest=false){if(!forceTest&&(!state.disease||state.disease.severity<65))return toast('Le 18 est réservé ici aux situations graves.');if(emergencyResponse)return toast('🚒 Une équipe de pompiers est déjà engagée pour toi.');if(state.interior){toast('🚨 Le centre 18 te demande de rejoindre l’entrée du bâtiment.');if(state.interior.shopType==='restaurant')restaurantSession=null;leaveInterior();if(state.interior)return toast('Impossible de localiser une sortie pour les secours.')}const patient={...state.pos};const start=res=>startFireResponse(res,patient,forceTest),res=reserveBestService('fire');if(!res){emergencyResponse={phase:'queued',vehicle:null,patient,forceTest,statusText:'Appel 18 en attente • toutes les équipes sont engagées'};dispatchQueued('fire',r=>{if(emergencyResponse?.phase==='queued')startFireResponse(r,patient,forceTest)});return}start(res)}
function startFireResponse(res,patient,forceTest=false){const u=createDispatchUnit('fire',res,patient,{speed:9.6,emergency:true});u.phase='toPatient';u.forceTest=forceTest;u.statusText=`VSAV ${res.mutualAid?'en renfort depuis '+(CITIES.find(c=>c.id===res.originCity)?.name||res.originCity):'en route depuis '+u.station.name} • ~${serviceEta(u)} s`;emergencyResponse=u;toast(`🚒 18 : ${u.statusText}`)}
function removeEmergencyVehicle(){removeServiceStretcher(emergencyResponse);if(emergencyResponse?.vehicle?.parent)scene.remove(emergencyResponse.vehicle);removeResponderCrew(emergencyResponse?.crew||[]);releaseServiceToken(emergencyResponse?.token);emergencyResponse=null;if(samuResponse?.vehicle?.parent)scene.remove(samuResponse.vehicle);removeResponderCrew(samuResponse?.crew||[]);releaseServiceToken(samuResponse?.token);samuResponse=null}
function startFireTransport(e,urgent=false){beginFireLoading(e,urgent)}
function requestSamuFromFire(e){if(samuResponse)return;const res=reserveBestService('samu');if(!res){e.phase='waitingSamu';e.statusText='Pompiers sur place • SAMU demandé, toutes les équipes sont engagées';dispatchQueued('samu',r=>{if(emergencyResponse===e&&e.phase==='waitingSamu')startSamuResponse(r,e)});return}startSamuResponse(res,e)}
function startSamuResponse(res,fire){const patient={...fire.patient};const u=createDispatchUnit('samu',res,patient,{speed:10.8,emergency:true});u.phase='toPatient';u.fire=fire;u.statusText=`SAMU ${res.mutualAid?'en renfort depuis '+(CITIES.find(c=>c.id===res.originCity)?.name||res.originCity):'en route'} • médecin + infirmier + ambulancier • ~${serviceEta(u)} s`;samuResponse=u;fire.phase='waitingSamu';fire.statusText=`Pompiers sur place • ${u.statusText}`;toast(`🚑 ${u.statusText}`)}
function finishHospitalArrival(hospital,via='fire'){removeServiceStretcher(emergencyResponse);if(emergencyResponse?.vehicle?.parent)scene.remove(emergencyResponse.vehicle);removeResponderCrew(emergencyResponse?.crew||[]);releaseServiceToken(emergencyResponse?.token);emergencyResponse=null;if(samuResponse?.vehicle?.parent)scene.remove(samuResponse.vehicle);removeResponderCrew(samuResponse?.crew||[]);releaseServiceToken(samuResponse?.token);samuResponse=null;state.hospitalFlow={admitted:true,step:'arrival',viaFirefighters:via==='fire',viaSamu:via==='samu'};enterInterior('shop',{type:'clinic',displayName:hospital.name,name:hospital.name},{preserveReturn:false});state.pos={x:0,z:5.5};toast(`🏥 Arrivée à ${hospital.name}. L’équipe médicale prend le relais.`);openSheet('physicalShop')}
function updateEmergencyResponse(dt){
 const e=emergencyResponse;if(!e)return;if(e.phase==='queued')return;const t=performance.now();
 if(e.phase==='toPatient')refreshServiceStatus(e,'VSAV en route');
 if(e.patientOnBoard){state.pos.x=e.vehicle.position.x;state.pos.z=e.vehicle.position.z}
 if(e.phase==='toPatient'||e.phase==='toHospital'||e.phase==='returning'){
   if(e.phase==='toHospital')refreshServiceStatus(e,`Transport vers ${e.hospital?.name||'hôpital'}`);
   const done=serviceDrive(e,dt,t);if(!done)return;
   if(e.phase==='toPatient'){
     parkServiceUnit(e);e.emergency=false;e.phase='deploying';e.deployTime=3.2;e.careActive=false;
     e.crew=spawnCrewForDeployment(e.vehicle,[{role:'firefighter',label:'Chef d’agrès'},{role:'firefighter',label:'Sapeur-pompier'},{role:'firefighter',label:'Sapeur-pompier'}]);
     e.statusText='VSAV garé • équipe en descente • ~3 s';toast('🚒 Le VSAV est garé près de toi. L’équipe descend du véhicule.');return
   }
   if(e.phase==='toHospital'){finishHospitalArrival(e.hospital,'fire');return}
   if(e.phase==='returning'){removeServiceStretcher(e);if(e.vehicle?.parent)scene.remove(e.vehicle);removeResponderCrew(e.crew);releaseServiceToken(e.token);emergencyResponse=null;return}
 }
 if(e.phase==='deploying'){
   e.deployTime=Math.max(0,(e.deployTime||0)-dt);const ready=updateCrewDeployment(e.crew,dt);e.statusText=`Équipe en descente • ~${Math.ceil(e.deployTime)} s`;
   if(ready||e.deployTime<=0){e.phase='confirm';e.statusText='Chef d’agrès vers toi • confirme ou refuse la prise en charge';toast('🚒 Le chef d’agrès vient te parler. Aucune prise en charge ne commence sans ton accord.')}return
 }
 if(e.phase==='confirm'){moveResponderCrewToward(e.crew,e.patient,dt,1.25);return}
 if(e.phase==='loading'){animateFireLoading(e,dt,t);return}
 if(e.phase==='assess'||e.phase==='waitingSamu'){
   if(e.phase==='waitingSamu'){animateCrewCare(e.crew,e.patient,dt,t);const se=samuResponse?.liveEta||serviceEta(samuResponse||{route:[],baseSpeed:1});e.statusText=samuResponse?`Bilan terminé • SAMU en route • ~${se} s`:'Bilan terminé • SAMU demandé, attente d’une équipe';return}
   e.careElapsed=(e.careElapsed||0)+dt;animateCrewCare(e.crew,e.patient,dt,t);e.dwell=Math.max(0,e.dwell-dt);e.statusText=`Bilan par les pompiers • ~${Math.ceil(e.dwell)} s`;
   if(e.dwell>0)return;
   if(e.forceTest&&!state.disease){e.statusText='Bilan terminé • aucune évacuation nécessaire';toast('🚒 « Le bilan est rassurant, aucune évacuation n’est nécessaire. »');removeResponderCrew(e.crew);removeServiceStretcher(e);e.phase='returning';e.careActive=false;e.stopPoint=nearestRoadPoint(e.station);e.route=emergencyRoute(e.vehicle.position,e.stopPoint);e.index=1;e.parkedSeg=null;e.vehicle.userData.serviceParked=false;return}
   const sev=state.disease?.severity||0;e.careActive=true;if(sev>=82){requestSamuFromFire(e);return}startFireTransport(e,sev>=72||state.hp<55)
 }
}
function updateSamuResponse(dt){const s=samuResponse;if(!s)return;const t=performance.now();if(s.phase==='toPatient')refreshServiceStatus(s,'SAMU en route');if(s.patientOnBoard){state.pos.x=s.vehicle.position.x;state.pos.z=s.vehicle.position.z}if(s.phase==='toPatient'||s.phase==='toHospital'||s.phase==='returning'){const done=serviceDrive(s,dt,t);if(!done)return;if(s.phase==='toPatient'){parkServiceUnit(s);s.emergency=false;s.phase='stabilize';s.dwell=7;s.crew=spawnCrewOutsideVehicle(s.vehicle,[{role:'doctor',label:'Médecin SAMU'},{role:'nurse',label:'Infirmier SAMU'},{role:'ambulance',label:'Ambulancier'}]);s.careActive=true;s.statusText='SAMU sur place • médicalisation en cours';toast('🚑 Le SAMU est sur place : médecin, infirmier et ambulancier commencent la stabilisation.');return}if(s.phase==='toHospital'){finishHospitalArrival(s.hospital,'samu');return}if(s.phase==='returning'){if(s.vehicle?.parent)scene.remove(s.vehicle);removeResponderCrew(s.crew);releaseServiceToken(s.token);samuResponse=null;return}}if(s.phase==='stabilize'){animateCrewCare(s.crew,s.target,dt,t);s.dwell-=dt;if(s.dwell>0)return;if(state.disease){state.disease.severity=clamp((state.disease.severity||0)-12,0,100);state.hp=clamp(state.hp+6,0,state.maxHp)}const critical=(state.disease?.severity||0)>=86||state.hp<45;if(critical){removeResponderCrew(s.crew);const f=s.fire;if(f){removeResponderCrew(f.crew);removeServiceStretcher(f);if(f.vehicle?.parent)scene.remove(f.vehicle);releaseServiceToken(f.token);emergencyResponse=null}s.patientOnBoard=true;s.careActive=false;s.phase='toHospital';s.emergency=true;s.hospital=nearestHospital(s.vehicle.position);if(!s.hospital){toast('🚑 Aucun hôpital disponible pour le transport médicalisé.');s.patientOnBoard=false;s.phase='returning';s.emergency=false;const dest=s.mutualAid?walkingEntryPosition(city(),mutualAidEntrySide(s.originCity,state.cityId)||'west',0):nearestRoadPoint(s.station);s.stopPoint=dest;s.route=emergencyRoute(s.vehicle.position,dest);s.index=1;s.parkedSeg=null;s.vehicle.userData.serviceParked=false;s.statusText='SAMU disponible • retour à sa base';return}s.stopPoint=nearestRoadPoint(s.hospital);s.route=emergencyRoute(s.vehicle.position,s.stopPoint);s.index=1;s.parkedSeg=null;s.vehicle.userData.serviceParked=false;s.statusText=`Patient stabilisé • transport médicalisé urgent vers ${s.hospital.name}`;toast('🚑 Le SAMU décide un transport médicalisé urgent.')}else{const f=s.fire;toast('🩺 État stabilisé par le SAMU. Les pompiers assurent le transport.');removeResponderCrew(s.crew);if(f)startFireTransport(f,(state.disease?.severity||0)>=70);s.phase='returning';s.emergency=false;const dest=s.mutualAid?walkingEntryPosition(city(),mutualAidEntrySide(s.originCity,state.cityId)||'west',0):nearestRoadPoint(s.station);s.stopPoint=dest;s.route=emergencyRoute(s.vehicle.position,dest);s.index=1;s.parkedSeg=null;s.vehicle.userData.serviceParked=false;s.statusText='SAMU disponible • retour à sa base'}}}
function policeEmergencyStatus(){if(!policeResponse)return'';if(policeResponse.phase==='queued')return'Appel 17 en attente';if(policeResponse.phase==='toCall')return policeResponse.statusText||'Police-secours en route';if(policeResponse.phase==='scene')return'Police sur place • sécurisation en cours';return''}
function makePatrolLoop(station,i){const c=nearestRoadPoint(station),d=CHUNK*(1+(i%2)),sgn=i%2?1:-1;return[{...c,road:true},{x:c.x+d*sgn,z:c.z,road:true},{x:c.x+d*sgn,z:c.z+d,road:true},{x:c.x-d*sgn,z:c.z+d,road:true},{x:c.x-d*sgn,z:c.z-d,road:true},{x:c.x,z:c.z-d,road:true},{...c,road:true}]}
function clearCityEmergencyServices(){removeEmergencyVehicle();clearPoliceResponse(true);for(const p of policePatrols){if(p.vehicle?.parent)scene.remove(p.vehicle);removeResponderCrew(p.officers||[]);releaseServiceToken(p.token)}policePatrols=[];serviceDispatchQueue=[];servicePools=null}
function initCityEmergencyServices(){if(!scene)return;for(const p of policePatrols){if(p.vehicle?.parent)scene.remove(p.vehicle)}policePatrols=[];ensureServicePools();const station=nearestCivic('policeStation');if(!station)return;const count=Math.max(1,serviceCapacity().police-backgroundBusy('police',state.cityId));for(let i=0;i<count;i++){const g=makePoliceResponseVehicle();flashEmergencyLights(g,0,false);const road=nearestRoadPoint(station);g.position.set(road.x+(i%2)*1.1,0,road.z+(i%2?1.2:-1.2));scene.add(g);const p={id:`patrol:${state.cityId}:${i}`,type:'police',vehicle:g,mode:'patrol',phase:'patrol',route:makePatrolLoop(station,i),index:1,baseSpeed:4.2+i*.25,currentSpeed:0,emergency:false,busy:false,temporary:false,station,officers:[],currentSeg:null};policePatrols.push(p)}}
function nearestIdlePolicePatrol(target){return policePatrols.filter(p=>!p.busy&&p.vehicle?.parent).map(p=>({p,d:routeLength(emergencyRoute(p.vehicle.position,nearestRoadPoint(target)))})).sort((a,b)=>a.d-b.d)[0]?.p||null}
function callPoliceEmergency(){if(policeResponse)return toast('👮 Une intervention police-secours est déjà ouverte.');if(state.interior){toast('📞 17 : la patrouille se rend à l’entrée du bâtiment.');if(state.interior.shopType==='restaurant')restaurantSession=null;leaveInterior();if(state.interior)return toast('Impossible de localiser une sortie pour la police.')}const target={...state.pos},patrol=nearestIdlePolicePatrol(target);if(patrol){startPolicePatrolResponse(patrol,target,false);return}const res=reserveMutualAidService('police');if(!res){policeResponse={phase:'queued',vehicle:null,statusText:'Appel 17 en attente • toutes les patrouilles sont engagées'};dispatchQueued('police',r=>{if(policeResponse?.phase==='queued')startMutualAidPolice(r,target)});return}startMutualAidPolice(res,target)}
function startPolicePatrolResponse(p,target,temporary=false){p.busy=true;p.mode='response';p.phase='toCall';p.emergency=true;p.target={...target};p.stopPoint=emergencyParkingPoint(target,'police');p.route=emergencyRoute(p.vehicle.position,p.stopPoint);p.index=1;p.baseSpeed=10.2;p.currentSpeed=Math.min(p.currentSpeed||0,4);p.dwell=0;p.officers=p.officers||[];p.parkedSeg=null;p.vehicle.userData.serviceParked=false;policeResponse=p;p.statusText=`Patrouille la plus proche en route • ~${Math.max(4,Math.ceil(routeLength(p.route)/p.baseSpeed))} s`;toast(`🚔 17 : ${p.statusText}`)}
function startMutualAidPolice(res,target){let v,station;if(res.mutualAid){v=serviceMutualAidSpawn(res,'police');station={name:`Renfort ${CITIES.find(c=>c.id===res.originCity)?.name||res.originCity}`}}else{const o=localServiceSpawn(res,'police');v=o.vehicle;station=o.station}const stop=emergencyParkingPoint(target,'police'),p={id:`police:reinforcement:${Date.now()}`,type:'police',token:res.token,vehicle:v,mode:'response',phase:'toCall',route:emergencyRoute(v.position,stop),index:1,baseSpeed:10.4,currentSpeed:0,emergency:true,busy:true,temporary:true,originCity:res.originCity,mutualAid:res.mutualAid,preDelay:res.delay||0,target:{...target},stopPoint:stop,station,officers:[],parkedSeg:null};policePatrols.push(p);policeResponse=p;p.statusText=`Renfort police ${res.mutualAid?'depuis '+(CITIES.find(c=>c.id===res.originCity)?.name||res.originCity):'depuis '+station.name} • ~${serviceEta(p)} s`;toast(`🚔 17 : ${p.statusText}`)}
function spawnResponseOfficers(r){if(r.officers?.length)return;r.officers=spawnCrewOutsideVehicle(r.vehicle,[{role:'police',label:'Chef de patrouille'},{role:'police',label:'Policier'}]);for(const n of r.officers)n.dispatchOfficer=true}
function clearPoliceResponse(force=false){if(!policeResponse)return;const r=policeResponse;removeResponderCrew(r.officers||[]);r.officers=[];if(r.temporary||force){if(r.vehicle?.parent)scene.remove(r.vehicle);releaseServiceToken(r.token);policePatrols=policePatrols.filter(x=>x!==r)}else{r.busy=false;r.mode='patrol';r.phase='patrol';r.emergency=false;r.baseSpeed=4.3;r.route=makePatrolLoop(r.station,Math.abs(hashStr(r.id))%3);r.index=1;r.currentSpeed=0;r.parkedSeg=null;r.vehicle.userData.serviceParked=false}policeResponse=null}
function updatePolicePatrols(dt,t){for(const p of [...policePatrols]){if(p.phase==='toCall')refreshServiceStatus(p,'Patrouille en route');if(!p.vehicle?.parent)continue;if(p.phase==='deploying'){flashEmergencyLights(p.vehicle,t,true);p.deployTime=Math.max(0,(p.deployTime||0)-dt);const ready=updateCrewDeployment(p.officers,dt);p.statusText=`Police sur place • agents en descente • ~${Math.ceil(p.deployTime)} s`;if(ready||p.deployTime<=0){p.phase='scene';p.dwell=22;p.statusText='Police sur place • sécurisation en cours';toast('🚔 Les policiers ont quitté le véhicule et sécurisent la zone.')}continue}if(p.phase==='scene'){flashEmergencyLights(p.vehicle,t,true);p.dwell-=dt;const nearby=enemies.filter(n=>n.group?.parent&&Math.hypot(n.group.position.x-p.vehicle.position.x,n.group.position.z-p.vehicle.position.z)<70);for(const n of nearby){n.policeFleeUntil=Math.max(n.policeFleeUntil||0,t+3000);n.policeFleeFrom={x:p.vehicle.position.x,z:p.vehicle.position.z}}for(let i=0;i<p.officers.length;i++){const o=p.officers[i],target=nearby[i%Math.max(1,nearby.length)]||p.target;if(target?.group){const dx=target.group.position.x-o.group.position.x,dz=target.group.position.z-o.group.position.z,d=Math.hypot(dx,dz)||1,k=Math.min(d,dt*2.2);o.group.position.x+=dx/d*k;o.group.position.z+=dz/d*k;setHeading(o,dx,dz)}else if(target)moveResponderCrewToward([o],target,dt,1.45)}if(p.dwell<=0){toast('👮 Intervention terminée • la patrouille reprend sa ronde.');clearPoliceResponse()}continue}const done=serviceDrive(p,dt,t);if(!done)continue;if(p.phase==='toCall'){parkServiceUnit(p);p.phase='deploying';p.deployTime=2.6;p.dwell=22;p.emergency=true;p.statusText='Police sur place • agents en descente';p.officers=spawnCrewForDeployment(p.vehicle,[{role:'police',label:'Chef de patrouille'},{role:'police',label:'Policier'}]);for(const n of p.officers)n.dispatchOfficer=true;for(const n of enemies){if(Math.hypot(n.group.position.x-p.vehicle.position.x,n.group.position.z-p.vehicle.position.z)<75){n.policeFleeUntil=t+20000;n.policeFleeFrom={x:p.vehicle.position.x,z:p.vehicle.position.z}}}toast('🚔 Police sur place • les individus hostiles prennent la fuite.')}else if(p.phase==='patrol'){p.index=1;p.route=makePatrolLoop(p.station,Math.abs(hashStr(p.id+Date.now()))%3);p.currentSpeed=0}}}
function updatePoliceResponse(dt,t){/* La conduite est gérée par updatePolicePatrols afin que patrouilles et interventions partagent la même physique. */}
...
function maybeContractFoodIllness(lot){
 if(!lot||lot.id==='water')return false;const spoiled=(lot.freshness??1)<=0;if(!spoiled)return false;
 const risky=['steak','chicken','sausage','sandwich','meal','dinner','milk','eggs','pastry'].includes(lot.id);const chance=risky?.72:.46;
 if(Math.random()>chance){toast('🤢 Ce produit était avarié… tu as eu de la chance cette fois.');return false}
 const added=24+Math.random()*32;if(state.disease?.id==='foodPoisoning')state.disease.severity=clamp((state.disease.severity||0)+added*.55,0,100);
 else state.disease={id:'foodPoisoning',severity:clamp(added,18,68),onset:gameStamp(),lastCheck:gameStamp(),diagnosed:false,lastDoseAt:null};
 toast('🤢 Tu commences à te sentir malade après avoir mangé cet aliment.');return true
}
function consumeFoodLot(uid,allowSpoiled=false){
 refreshFoodSpoilage();const lot=(state.foodLots||[]).find(x=>x.uid===uid&&x.location==='bag');if(!lot)return;
 const bad=(lot.freshness??1)<=0;if(bad&&!allowSpoiled)return toast('Cet aliment est avarié.');if(RAW_FOODS.has(lot.id))return toast('Cet aliment doit être cuit avant consommation.');
 const c=CONSUMABLES[lot.id];removeFoodLot(uid);state.hunger=clamp(state.hunger+(c?.hunger||0),0,100);state.thirst=clamp(state.thirst+(c?.thirst||0),0,100);if(bad)maybeContractFoodIllness(lot);save();openSheet('bag')
}
function takePrescriptionDose(rxId){
 const rx=(state.prescriptions||[]).find(x=>x.uid===rxId&&x.active!==false);if(!rx)return toast('Ordonnance introuvable.');if((state.medications?.[rx.medicineId]||0)<=0)return toast('Tu dois d’abord récupérer les médicaments à la pharmacie.');
 const now=gameStamp();if(now+0.01<rx.nextDoseAt)return toast(`Trop tôt : ${prescriptionDueText(rx)}.`);
 state.medications[rx.medicineId]--;rx.remainingDoses=Math.max(0,rx.remainingDoses-1);rx.nextDoseAt=now+rx.doseEveryHours/24;if(state.disease?.id===rx.diseaseId){state.disease.severity=clamp(state.disease.severity-(6+Math.random()*5),0,100);state.disease.lastDoseAt=now}
 if(rx.remainingDoses<=0){rx.active=false;if(state.disease&&state.disease.severity<18){state.disease=null;toast('✅ Traitement terminé : tu te sens rétabli.')}else toast('💊 Dernière prise effectuée. Le traitement est terminé.')}
 else toast(`💊 Prise effectuée • ${rx.remainingDoses} restante(s).`);save();openSheet(currentPanel==='home'?'home':'bag')
}
function movePrescription(uid,to){const rx=(state.prescriptions||[]).find(x=>x.uid===uid);if(!rx)return;if(to==='home'&&!atOwnResidence())return toast('Il faut être chez toi pour ranger l’ordonnance.');rx.location=to;save();openSheet(to==='home'?'home':'bag')}
function updateDisease(dt){
 if(!state.disease)return;const now=gameStamp(),last=state.disease.lastCheck??now,elapsedH=Math.max(0,(now-last)*24);if(elapsedH<.05)return;state.disease.lastCheck=now;
 const protectedCare=!!(emergencyResponse?.patientOnBoard||emergencyResponse?.careActive||samuResponse?.careActive||samuResponse?.patientOnBoard||state.hospitalFlow?.admitted),factor=protectedCare?.18:1;const sev=state.disease.severity||0;
 // Natural course deliberately fluctuates: mild cases often improve, serious ones can worsen.
 let drift=((Math.random()-.48)*(sev>65?2.4:1.8)-(.20*(sev<42?1:0)))*elapsedH*factor;
 const rx=currentPrescription();if(rx&&state.disease.lastDoseAt&&((now-state.disease.lastDoseAt)*24)<rx.doseEveryHours*1.35)drift-=.45*elapsedH;
 state.disease.severity=clamp(sev+drift,0,100);
 if(state.disease.severity>48){const loss=elapsedH*((state.disease.severity-42)/58)*.16*factor;state.hp=Math.max(1,state.hp-loss)}
 if(state.disease.severity<=1.5&&elapsedH>.2){state.disease=null;toast('🙂 Les symptômes ont fini par disparaître spontanément.');save()}
}
function healthStatusHTML(){
 if(!state.disease)return`<div class="card healthOkay"><h3>❤️ Santé</h3><p class="sub">Aucune maladie active.</p></div>`;const d=diseaseDef(),rx=currentPrescription(),canCall=(state.disease.severity||0)>=65&&!emergencyResponse;return`<div class="card sickCard"><div class="sectionKicker">ÉTAT DE SANTÉ</div><h3>${d.icon} ${d.name}</h3><p class="sub">Évolution ${diseaseStage()} • intensité ${Math.round(state.disease.severity)}%. La maladie peut s’améliorer ou s’aggraver naturellement. Les kits de soin ne guérissent pas une maladie.</p>${rx?prescriptionHTML(rx,false):'<p class="sub">Aucun traitement prescrit. Consulte un médecin ou un hôpital.</p>'}${rx&&activeMedicineCount(rx.medicineId)>0?`<button class="menuBtn primary takeDose" data-rx="${rx.uid}">💊 Prendre la dose • ${activeMedicineCount(rx.medicineId)} en stock</button>`:''}${canCall?'<button class="menuBtn red callFirefighters" style="width:100%">🚒 Appeler les pompiers</button>':''}${emergencyResponse?`<p class="sub"><b>🚨 Secours engagés :</b> ${emergencyResponse.statusText||'en route'}</p>`:''}</div>`
}
function medicalPlaceKind(){const n=(state.interior?.shopName||'').toLowerCase();return /cabinet|centre de santé/.test(n)&&!/hôpital|clinique/.test(n)?'doctor':'hospital'}
function startDoctorConsultation(){
 if(!state.disease)return toast('Tu n’as pas de maladie active.');const kind=medicalPlaceKind(),d=diseaseDef(),cost=kind==='hospital'?d.hospitalCost:d.doctorCost;
 if(kind==='doctor'&&state.coins<cost)return toast(`Consultation : ${cost} crédits.`);
 if(kind==='hospital'){const paid=Math.min(state.coins,cost);state.coins-=paid;const debt=cost-paid;state.medicalDebt=(state.medicalDebt||0)+debt;if(debt)toast(`🏥 Les soins ne sont pas refusés : ${debt} crédits restent dus à l’hôpital.`)}else state.coins-=cost;
 state.medicalBills=(state.medicalBills||0)+cost;state.disease.diagnosed=true;advanceGameMinutes(kind==='hospital'?55:30);const rx=createPrescription(kind==='hospital'?'hospital':'doctor');
 if(kind==='hospital'){state.hospitalFlow={admitted:true,step:'examined',rxId:rx.uid};toast('🩺 « Bonjour, je suis le médecin. Je vais vous examiner. » Une ordonnance est préparée et une infirmière va administrer la première dose.')}else toast('🩺 « Je vous prescris ce traitement. » L’ordonnance est dans ton sac : va à la pharmacie pour récupérer les médicaments.');save();openSheet('physicalShop')
}
function hospitalNurseDose(){const flow=state.hospitalFlow,rx=(state.prescriptions||[]).find(x=>x.uid===flow?.rxId);if(!flow||!rx)return;if((state.medications[rx.medicineId]||0)<1)state.medications[rx.medicineId]=(state.medications[rx.medicineId]||0)+1;state.medications[rx.medicineId]--;rx.remainingDoses=Math.max(0,rx.remainingDoses-1);rx.nextDoseAt=gameStamp()+rx.doseEveryHours/24;if(state.disease?.id===rx.diseaseId){state.disease.lastDoseAt=gameStamp();state.disease.severity=clamp(state.disease.severity-(14+Math.random()*5),0,100)}flow.step='stabilized';flow.admitted=true;toast('👩‍⚕️ L’infirmière administre la première dose prescrite.');save();openSheet('physicalShop')}
function hospitalDischarge(){if(!state.hospitalFlow)return;state.hospitalFlow=null;save();toast('🏥 Le médecin autorise ta sortie avec ton ordonnance.');openSheet('physicalShop')}
function pharmacyHTML(name,staff){
 const active=(state.prescriptions||[]).filter(x=>x.active!==false);return`${staff}<div class="card"><div class="sectionKicker">PHARMACIE</div><h3>💊 ${name}</h3><p class="sub">Les médicaments sur ordonnance sont délivrés selon la prescription. Acheter davantage ne permet pas de dépasser la posologie.</p></div>${active.length?active.map(rx=>{const m=MEDICINES[rx.medicineId],missing=Math.max(0,rx.remainingDoses-(state.medications?.[rx.medicineId]||0)),price=missing*(m?.pricePerDose||2);return`${prescriptionHTML(rx,false)}<div class="card"><div class="lifeStat"><div><b>${m?.icon||'💊'} ${m?.name||rx.medicineId}</b><small>${missing?`${missing} dose(s) à délivrer`:'Traitement déjà récupéré'}</small></div><button class="menuBtn pharmacyFill" data-rx="${rx.uid}" ${missing?'':'disabled'}>${missing?`Acheter • ${price} cr.`:'Délivré'}</button></div></div>`}).join(''):'<div class="card"><p class="sub">Aucune ordonnance active.</p></div>'}<button class="menuBtn red" id="leaveShop" style="width:100%">🚪 Sortir</button>`
}
function fillPrescription(rxId){const rx=(state.prescriptions||[]).find(x=>x.uid===rxId&&x.active!==false);if(!rx)return;const m=MEDICINES[rx.medicineId],missing=Math.max(0,rx.remainingDoses-(state.medications?.[rx.medicineId]||0)),price=missing*(m?.pricePerDose||2);if(!missing)return toast('Traitement déjà délivré.');if(state.coins<price)return toast(`Il faut ${price} crédits.`);state.coins-=price;state.medications[rx.medicineId]=(state.medications[rx.medicineId]||0)+missing;recordBusinessTransaction(price,'pharmacy');advanceGameMinutes(8);save();toast('💊 Médicaments délivrés. Respecte la posologie de l’ordonnance.');openSheet('physicalShop')}
...
function updateNeeds(dt){
 const calm=state.interior?(state.interior.type==='shop'?.12:.38):1;state.hunger=clamp(state.hunger-dt*.0085*calm,0,100);state.thirst=clamp(state.thirst-dt*.0120*calm,0,100);state.hygiene=clamp(state.hygiene-dt*.0050*calm,0,100);state.dentalHygiene=clamp((state.dentalHygiene??70)-dt*.0018*calm,0,100);refreshFoodSpoilage();updateDisease(dt);if(state.hunger<=0||state.thirst<=0)state.hp=Math.max(1,state.hp-dt*(state.thirst<=0?.34:.18))
}
function useConsumable(id){
 const c=CONSUMABLES[id];if(!c)return;if(RAW_FOODS.has(id))return toast('Cet aliment doit être cuisiné avant d’être consommé.');
 if(FOOD_META[id]){refreshFoodSpoilage();const lot=(state.foodLots||[]).filter(x=>x.id===id&&x.location==='bag'&&(x.freshness??1)>0).sort((a,b)=>(a.freshness??1)-(b.freshness??1))[0];if(!lot)return toast('Aucun exemplaire consommable dans ton sac.');return consumeFoodLot(lot.uid,false)}
 if(!removeStack(state.inventory,id,1))return;state.hunger=clamp(state.hunger+(c.hunger||0),0,100);state.thirst=clamp(state.thirst+(c.thirst||0),0,100);state.hygiene=clamp(state.hygiene+(c.hygiene||0),0,100);toast(`${c.icon} ${c.name} consommé`);save();openSheet('bag')
}
...
function animate(){
 ...
 const moveSpeed=4.8*needsSpeedMultiplier();if(!busRide&&!emergencyResponse?.patientOnBoard&&!emergencyResponse?.careActive&&!samuResponse?.careActive&&!samuResponse?.patientOnBoard){movePlayer((fx*forward+rx*strafe)*moveSpeed*dt,(fz*forward+rz*strafe)*moveSpeed*dt);updatePlayerTrail()}
 ...
 updateNeeds(dt);try{updateServiceQueues(t)}catch(err){console.error('Service queue frame',err)}try{updateEmergencyResponse(dt)}catch(err){console.error('Fire response frame',err)}try{updateSamuResponse(dt)}catch(err){console.error('SAMU response frame',err)}try{updatePolicePatrols(dt,t)}catch(err){console.error('Police patrol frame',err)}try{updatePoliceResponse(dt,t)}catch(err){console.error('Police response frame',err)}updateRestaurantService(dt);updateWorkMission();if(state.interior)updateInteriorAmbientPeople(t);if(!state.interior){try{updatePeople(dt,t)}catch(err){console.error('People frame',err)}try{updateCars(dt,t);maintainTrafficPopulation(t)}catch(err){console.error('Traffic frame',err)}try{updateBusVehicles(dt)}catch(err){console.error('Bus frame',err)}animatePickups(dt,t);if(t-lastChunkTick>650){try{ensureChunks()}catch(err){console.error('Chunk refresh',err)}lastChunkTick=t}}updateCamera(t);updateWorldLight(dt);try{updateParisLampLights(t)}catch(err){console.error('Lamp-light frame error',err)}updateAtmosphere(dt);checkInteraction();if(t-lastMapTick>180){drawMap();lastMapTick=t}if(t-lastHudTick>100){updateHUD();lastHudTick=t}renderer.render(scene,camera);try{multiplayerTick(t,dt)}catch(err){console.error('Multiplayer frame error',err)}
}
...
function checkInteraction(){
 ...
 const fireConfirm=emergencyResponse?.phase==='confirm'?emergencyResponse:null;if(fireConfirm?.crew?.[0]?.group?.parent){const leader=fireConfirm.crew[0],d=Math.hypot(state.pos.x-leader.group.position.x,state.pos.z-leader.group.position.z);if(d<3.4)return setPrompt('🚒 Chef d’agrès','Les pompiers attendent que tu confirmes ou refuses la prise en charge.','PARLER',()=>openSheet('emergencyConfirm'))}
 const careStatus=emergencyResponse&&!['toPatient','confirm','returning','toHospital'].includes(emergencyResponse.phase)?emergencyResponse:null;if(careStatus)return setPrompt('🚑 Intervention en cours',careStatus.statusText||'Prise en charge en cours','SUIVI',()=>openSheet('phone'));
...
function phoneContactNumber(seed){let h=2166136261;for(const c of String(seed||'npc')){h^=c.charCodeAt(0);h=Math.imul(h,16777619)}h=Math.abs(h>>>0);const digits=String(h%100000000).padStart(8,'0');return `06 ${digits.slice(0,2)} ${digits.slice(2,4)} ${digits.slice(4,6)} ${digits.slice(6,8)}`}
function learnNpcOccupation(n){if(!n||npcOccupationKnown(n))return;if(!state.knownNpcOccupations.includes(n.npcId))state.knownNpcOccupations.push(n.npcId);state.phoneContacts=state.phoneContacts||[];if(!state.phoneContacts.some(c=>c.npcId===n.npcId))state.phoneContacts.push({npcId:n.npcId,name:n.name||'Contact',occupation:n.occupation?.title||'Habitant',number:phoneContactNumber(n.npcId)});save()}
...
function addInteriorAmbientPerson(role,x,z,rot=0,label=false){const g=buildRemoteAvatarMesh({name:'',avatar:interiorRoleAvatar(role,interiorAmbientPeople.length)});g.position.set(x,0,z);g.rotation.y=rot;interiorGroup.add(g);interiorAmbientPeople.push({group:g,baseY:0,phase:interiorAmbientPeople.length*.8,role});return g}
...
function leaveInterior(){if(state.interior?.shopType==='restaurant'&&restaurantSession&&['cooking','served','eaten'].includes(restaurantSession.phase))return toast('🍽️ Termine le service et règle l’addition avant de partir.');restaurantSession=null;
...
function serviceBuildingHTML(type){const name=state.interior?.shopName||SHOPS[type].name,staff=staffingCardHTML(type,name);if(type==='clinic'){const kind=medicalPlaceKind(),flow=state.hospitalFlow,d=state.disease;let care='';if(!d)care='<div class="card"><p class="sub">Aucune maladie active. Les urgences restent ouvertes 24 h/24.</p></div>';else if(kind==='doctor')care=`<div class="card"><div class="sectionKicker">CABINET MÉDICAL</div><h3>🩺 Consultation</h3><p class="sub">Le médecin t’examine. Il ne délivre pas directement les médicaments : il rédige une ordonnance à présenter à la pharmacie.</p><button class="menuBtn primary doctorConsult" style="width:100%">Voir le médecin • ${diseaseDef().doctorCost} cr.</button></div>`;else if(flow?.step==='arrival')care=`<div class="card sickCard"><h3>🚑 Admission par les pompiers</h3><p class="sub">Un médecin vient te parler, t’interroger et t’examiner. Pendant la prise en charge, l’évolution de la maladie est fortement ralentie.</p><button class="menuBtn primary doctorConsult">Parler au médecin et être examiné</button></div>`;else if(flow?.step==='examined')care=`<div class="card"><h3>👩‍⚕️ Traitement hospitalier</h3><p class="sub">Le médecin a rédigé l’ordonnance. Une infirmière vient administrer la première prise.</p><button class="menuBtn primary hospitalNurseDose">Recevoir la première dose</button></div>`;else if(flow?.step==='stabilized')care=`<div class="card"><h3>🩺 Sortie médicale</h3><p class="sub">Le médecin te réévalue. Ton ordonnance reste valable pour poursuivre le traitement plusieurs jours.</p><button class="menuBtn hospitalDischarge">Le médecin autorise la sortie</button></div>`;else care=`<div class="card"><div class="sectionKicker">HÔPITAL</div><h3>${diseaseLabel()}</h3><p class="sub">Consultation, examen et prescription. L’hôpital est payant ; les pompiers ne le sont pas.</p><button class="menuBtn primary doctorConsult" style="width:100%">Consulter • ${diseaseDef().hospitalCost} cr.</button></div>`;return`${staff}${care}`}
 if(type==='policeStation')return`${staff}<div class="card"><div class="sectionKicker">SÉCURITÉ PUBLIQUE</div><h3>👮 ${name}</h3><p class="sub">Accueil 24 h/24 • patrouilles, police-secours et personnel administratif.</p><p class="sub">Niveau de recherche actuel : <b>${state.wanted||0}</b>.</p>${state.wanted?`<button class="menuBtn policeSettle" style="width:100%">Se présenter et régler la situation</button>`:'<span class="qualification done">Aucune recherche en cours</span>'}</div>`;
 if(type==='fireStation')return`${staff}<div class="card"><div class="sectionKicker">SECOURS</div><h3>🚒 ${name}</h3><p class="sub">Caserne opérationnelle 24 h/24 : gardes, départs incendie et secours à personne. Les interventions médicales d’urgence sont gratuites.</p></div>`;
 if(type==='townhall')return`${staff}${societyDashboardHTML()}<div class="card"><h3>🏛️ ${name}</h3><p class="sub">État civil, accueil des habitants, services municipaux, voirie et administration.</p></div>`;if(type==='bank')return`${staff}<div class="card"><div class="sectionKicker">BANQUE</div><h3>🏦 ${name}</h3><p class="sub">Conseillers, chargés d’accueil et clientèle sont visibles dans l’agence.</p><p class="sub">Compte : <b>${state.homeBank}</b> crédits • portefeuille : ${state.coins}.</p><div class="grid2"><button class="menuBtn bankDeposit">Déposer 50</button><button class="menuBtn bankWithdraw">Retirer 50</button></div></div>`;if(type==='postoffice')return`${staff}<div class="card"><div class="sectionKicker">SERVICE POSTAL</div><h3>📮 ${name}</h3><p class="sub">Guichets, tri local, courrier, colis et services de proximité.</p><button class="menuBtn postalParcel" style="width:100%">📦 Envoyer un colis • 5 cr.</button></div>`;if(type==='library')return`${staff}<div class="card"><div class="sectionKicker">CULTURE & ÉTUDE</div><h3>📚 ${name}</h3><p class="sub">Bibliothécaires, lecteurs et étudiants occupent réellement les espaces de lecture.</p><button class="menuBtn libraryStudy" style="width:100%">📖 Étudier 2 heures</button></div>`;return staff}
...
function hospitalCare(){startDoctorConsultation()}
...
function counterFoodShopHTML(type,name,staff){const s=SHOPS[type],mult=localPriceMultiplier();return`${staff}<div class="card"><div class="sectionKicker">VENTE AU COMPTOIR</div><h3>${s.icon} ${name}</h3><p class="sub">Ici il n’y a pas de libre-service : approche-toi du comptoir et commande au vendeur, comme dans une vraie ${type==='bakery'?'boulangerie':'boucherie'}.</p>${s.stock.filter(x=>FOOD_META[x.id]).map(x=>`<div class="item"><div class="itemIcon">${x.icon}</div><div class="itemMain"><b>${x.name}</b><small>${Math.max(1,Math.round(x.price*mult))} cr.</small></div><button class="menuBtn counterBuy" data-id="${x.id}" data-price="${x.price}">Commander</button></div>`).join('')}</div><button class="menuBtn red" id="leaveShop" style="width:100%">🚪 Sortir</button>`}
function counterBuy(id,price){const final=Math.max(1,Math.round(Number(price)*localPriceMultiplier()));if(invCount()>=state.bagMax)return toast('Sac plein.');if(state.coins<final)return toast('Pas assez de crédits.');state.coins-=final;addFoodLot(id,'bag',1);recordBusinessTransaction(final,state.interior?.shopType);advanceGameMinutes(3);save();toast(`${CONSUMABLES[id]?.icon||'🛍️'} Le vendeur te remet ${CONSUMABLES[id]?.name||id}.`);openSheet('physicalShop')}
function restaurantEnsure(){const name=state.interior?.shopName||'Restaurant';if(!restaurantSession||restaurantSession.name!==name)restaurantSession={name,phase:'idle',order:null,price:0,waiterTarget:null};return restaurantSession}
function restaurantHTML(name,staff){const r=restaurantEnsure();let body='';if(r.phase==='idle')body=`<div class="card"><div class="sectionKicker">SERVICE À TABLE</div><h3>🍽️ ${name}</h3><p class="sub">Installe-toi. Un serveur viendra prendre ta commande puis transmettra en cuisine.</p><button class="menuBtn primary restaurantSit" style="width:100%">🪑 S’asseoir à une table</button></div>`;else if(r.phase==='waitingMenu')body=`<div class="card"><h3>🧑‍🍳 Le serveur arrive…</h3><p class="sub">Tu es installé. Le serveur se dirige vers ta table.</p></div>`;else if(r.phase==='menu')body=`<div class="card"><h3>📖 La carte</h3><p class="sub">Le serveur attend ta commande.</p>${RESTAURANT_MENU.map(x=>`<div class="item"><div class="itemIcon">${x.icon}</div><div class="itemMain"><b>${x.name}</b><small>${x.price} cr.</small></div><button class="menuBtn restaurantOrder" data-id="${x.id}">Commander</button></div>`).join('')}</div>`;else if(r.phase==='cooking')body=`<div class="card"><h3>👨‍🍳 En cuisine</h3><p class="sub">Le serveur a transmis « ${r.order?.name} » au cuisinier. Le plat est en préparation.</p><div class="progress"><i style="width:65%"></i></div></div>`;else if(r.phase==='served')body=`<div class="card"><h3>🍽️ ${r.order?.name}</h3><p class="sub">Le serveur vient de déposer le plat à ta table.</p><button class="menuBtn primary restaurantEat">Manger</button></div>`;else if(r.phase==='eaten')body=`<div class="card"><h3>🧾 Addition</h3><p class="sub">Repas terminé. Il reste à régler ${r.price} crédits avant de partir.</p><button class="menuBtn primary restaurantPay">Payer • ${r.price} cr.</button></div>`;else body=`<div class="card"><h3>✅ Merci</h3><p class="sub">L’addition est réglée. Tu peux quitter le restaurant.</p></div>`;return`${staff}${body}<button class="menuBtn red" id="leaveShop" style="width:100%" ${['cooking','served','eaten'].includes(r.phase)?'disabled':''}>🚪 Sortir</button>`}
function restaurantSit(){const r=restaurantEnsure();r.phase='waitingMenu';r.waiterTarget={x:1.8,z:1.4};state.pos={x:0,z:2.0};openSheet('physicalShop');setTimeout(()=>{if(!restaurantSession||restaurantSession.phase!=='waitingMenu'||state.interior?.shopType!=='restaurant')return;restaurantSession.phase='menu';restaurantSession.waiterTarget={x:.9,z:1.2};openSheet('physicalShop')},1500)}
function restaurantOrder(id){const item=RESTAURANT_MENU.find(x=>x.id===id);if(!item)return;const r=restaurantEnsure();r.order=item;r.price=Math.max(1,Math.round(item.price*localPriceMultiplier()));r.phase='cooking';r.waiterTarget={x:0,z:-6.2};openSheet('physicalShop');setTimeout(()=>{if(!restaurantSession||restaurantSession.phase!=='cooking'||state.interior?.shopType!=='restaurant')return;restaurantSession.phase='served';restaurantSession.waiterTarget={x:1.4,z:1.3};openSheet('physicalShop');toast('🍽️ Le serveur apporte ton plat.')},4200)}
function restaurantEat(){const r=restaurantEnsure();if(r.phase!=='served'||!r.order)return;const c=CONSUMABLES[r.order.id];state.hunger=clamp(state.hunger+(c?.hunger||35),0,100);state.thirst=clamp(state.thirst+(c?.thirst||0),0,100);advanceGameMinutes(r.order.minutes||35);r.phase='eaten';r.waiterTarget={x:1.3,z:1.4};save();openSheet('physicalShop')}
function restaurantPay(){const r=restaurantEnsure();if(r.phase!=='eaten')return;if(state.coins<r.price)return toast(`Il faut ${r.price} crédits.`);state.coins-=r.price;recordBusinessTransaction(r.price,'restaurant');r.phase='paid';save();toast('🧾 Addition réglée.');openSheet('physicalShop')}
function updateRestaurantService(dt){if(!restaurantSession?.waiterTarget||state.interior?.shopType!=='restaurant'||!interiorSeller?.group)return;const g=interiorSeller.group,t=restaurantSession.waiterTarget,dx=t.x-g.position.x,dz=t.z-g.position.z,d=Math.hypot(dx,dz);if(d>.08){const k=Math.min(d,dt*2.2);g.position.x+=dx/d*k;g.position.z+=dz/d*k;g.rotation.y=Math.atan2(dx,dz)}}
...
function physicalShopHTML(){
 ...
 if(['clinic','policeStation','fireStation','townhall','bank','postoffice','library'].includes(type))return `${serviceBuildingHTML(type)}<button class="menuBtn red" id="leaveShop" style="width:100%" ${type==='clinic'&&state.hospitalFlow?.admitted?'disabled':''}>🚪 Sortir</button>`;
 ...
 if(type==='pharmacy')return pharmacyHTML(name,staff);if(type==='restaurant')return restaurantHTML(name,staff);if(COUNTER_FOOD_TYPES.has(type))return counterFoodShopHTML(type,name,staff);if(FOOD_SHOP_TYPES.has(type))return groceryShopHTML(type,name,staff);
...
function bindShop(){
 ...
 $$('.cartAdd').forEach(b=>b.onclick=()=>addToCart(b.dataset.id));$$('.counterBuy').forEach(b=>b.onclick=()=>counterBuy(b.dataset.id,b.dataset.price));$$('.pharmacyFill').forEach(b=>b.onclick=()=>fillPrescription(b.dataset.rx));$('.doctorConsult')?.addEventListener('click',startDoctorConsultation);$('.hospitalNurseDose')?.addEventListener('click',hospitalNurseDose);$('.hospitalDischarge')?.addEventListener('click',hospitalDischarge);$('.restaurantSit')?.addEventListener('click',restaurantSit);$$('.restaurantOrder').forEach(b=>b.onclick=()=>restaurantOrder(b.dataset.id));$('.restaurantEat')?.addEventListener('click',restaurantEat);$('.restaurantPay')?.addEventListener('click',restaurantPay);$$('.cartRemove').forEach(b=>b.onclick=()=>removeFromCart(b.dataset.id));$('.checkoutCart')?.addEventListener('click',checkoutCart);
...
function renderMapTo(canvas,zoom=2.0){
 ...
 if(mapPhoneTarget){const dx=(mapPhoneTarget.x-center.x)*S,dz=(mapPhoneTarget.z-center.z)*S;if(Math.abs(dx)<W/2&&Math.abs(dz)<H/2){drawMapMarker(q,dx,dz,'★','#ffef83',detail);if(detail){q.fillStyle='#ffef83';q.font='800 10px system-ui';q.textAlign='left';q.fillText(mapPhoneTarget.label||'Destination',dx+12,dz-10)}}}
...
function updateHUD(){
 ...
 $('#hp').textContent=Math.round(state.hp);const hpChip=$('#hp')?.closest('span');if(hpChip){hpChip.classList.toggle('sickHealth',!!state.disease);hpChip.title=state.disease?diseaseLabel():'Santé'}$('#armor').textContent=Math.round(state.armor);$('#coins').textContent=state.coins;
 ...
 // V22.9 : plus de badge POLICE flottant sur le HUD ; l'état des secours reste dans StreetPhone.
...
function phoneOutdoorPos(){return state.interior?(state.returnPos||state.pos):state.pos}
function showPhoneMapPoint(x,z,label='Destination'){closeSheet();mapWorldMode=false;mapBusMode=false;mapFocusPropertyId=null;mapPhoneTarget={x:Number(x),z:Number(z),label};mapCenterOverride={x:Number(x),z:Number(z)};bigMapZoom=.92;$('#mapOverlay').classList.remove('hidden');drawMap()}
function phoneAppTile(app,icon,name,sub=''){return `<button class="phoneAppTile" data-phone-app="${app}"><span class="phoneAppIcon">${icon}</span><b>${name}</b><small>${sub}</small></button>`}
function phoneHomeHTML(){const fire=emergencyResponse?.statusText||'',samu=samuResponse?.statusText||'',pol=policeEmergencyStatus();return `<div class="phoneShell"><div class="phoneTop"><span>StreetPhone</span><b>${formatGameTime(state.timeOfDay)}</b><span>${city().name}</span></div><div class="phoneGrid">${phoneAppTile('phoneMap','🗺️','Carte','Ville & région')}${phoneAppTile('phoneAgenda','📅','Agenda','Rendez-vous')}${phoneAppTile('phoneGuide','📖','Guide','Comment jouer')}${phoneAppTile('phoneDirectory','📍','Annuaire','Autour de moi')}${phoneAppTile('phoneContacts','👤','Contacts','Rencontres')}${phoneAppTile('phoneDialer','☎️','Téléphone','17 / 18')}${phoneAppTile('phoneTransport','🚌','Transports','Bus & train')}</div>${fire||samu||pol?`<div class="phoneEmergencyLive"><b>Intervention en cours</b>${fire?`<small>🚒 ${fire}</small>`:''}${samu?`<small>🚑 ${samu}</small>`:''}${pol?`<small>🚔 ${pol}</small>`:''}</div>`:''}</div>`}
function phoneBack(){openSheet('phone')}
function phoneBackButton(){return `<button class="phoneBack menuBtn">‹ Téléphone</button>`}
function phoneGuideHTML(){return `${phoneBackButton()}<div class="card"><div class="sectionKicker">GUIDE STREETQUEST</div><h3>📖 Bien démarrer</h3><p class="sub"><b>Se déplacer :</b> marche avec le joystick gauche et regarde autour de toi avec le droit. La carte et les transports permettent de s’orienter entre les quartiers et les villes.</p><p class="sub"><b>Besoins :</b> mange, bois, dors, prends une douche et prends soin de ton hygiène. Les aliments ont une durée de conservation et peuvent rendre malade s’ils sont avariés.</p><p class="sub"><b>Travail :</b> consulte la Maison de l’Emploi, respecte tes horaires et utilise ton uniforme professionnel seulement quand tu travailles.</p><p class="sub"><b>Logement :</b> location meublée ou non meublée ; un achat est livré non meublé. Ton armoire peut stocker tes uniformes et ordonnances.</p><p class="sub"><b>Santé :</b> médecin → ordonnance → pharmacie. En urgence grave, appelle le 18. En cas de danger ou d’agression, appelle le 17.</p><p class="sub"><b>Transports :</b> attends physiquement le bus à son arrêt, monte pendant son temps d’arrêt et choisis où descendre. Les trains relient les villes.</p></div>`}
function phoneDirectoryEntries(){const p=phoneOutdoorPos(),out=[],seen=new Set();const add=(x)=>{if(!x||x.cityId&&x.cityId!==state.cityId)return;const key=`${x.type}:${Math.round(x.x)}:${Math.round(x.z)}`;if(seen.has(key))return;seen.add(key);out.push({...x,d:Math.hypot(p.x-x.x,p.z-x.z)})};for(const x of CIVIC_POIS)add({...x,cityId:state.cityId});for(const s of state.discoveredShops||[])add(s);for(const s of shops||[])add({type:s.type,name:s.displayName||SHOPS[s.type]?.name||'Commerce',x:s.door?.x??s.x,z:s.door?.z??s.z,cityId:state.cityId});return out.sort((a,b)=>a.d-b.d).slice(0,30)}
function phoneDirectoryHTML(){const list=phoneDirectoryEntries();return `${phoneBackButton()}<div class="card"><div class="sectionKicker">AUTOUR DE MOI</div><h3>📍 Annuaire</h3><p class="sub">Horaires, distance et localisation des commerces, médecins, pharmacies et services publics proches.</p></div>${list.map(x=>{const st=mapShopStyle(x.type),open=shopIsOpen(x.type);return `<div class="card phoneDirectoryRow"><div class="phonePlaceIcon" style="--pc:${st.color}">${st.code}</div><div><b>${x.name||SHOPS[x.type]?.name||st.label}</b><small>${st.label} • ${Math.round(x.d)} m</small><small class="${open?'openNow':'closedNow'}">${open?'Ouvert':'Fermé'} • ${shopHoursLabel(x.type)}</small></div><button class="tinyBtn phoneMapPlace" data-x="${x.x}" data-z="${x.z}" data-name="${encodeURIComponent(x.name||st.label)}">Carte</button></div>`}).join('')||'<div class="card"><p class="sub">Aucun établissement connu autour de toi. Explore le quartier pour enrichir l’annuaire.</p></div>'}`}
function phoneContactsHTML(){const contacts=state.phoneContacts||[];return `${phoneBackButton()}<div class="card"><div class="sectionKicker">NUMÉROS UTILES</div><h3>👤 Contacts</h3><div class="phoneContact"><b>🚔 Police-secours</b><small>17 • urgence et danger</small><button class="tinyBtn quickCall" data-number="17">Appeler</button></div><div class="phoneContact"><b>🚒 Sapeurs-pompiers</b><small>18 • secours grave</small><button class="tinyBtn quickCall" data-number="18">Appeler</button></div></div><div class="card"><h3>Mes rencontres</h3>${contacts.length?contacts.map(c=>`<div class="phoneContact"><b>👤 ${c.name}</b><small>${c.occupation||'Habitant'} • ${c.number||phoneContactNumber(c.npcId)}</small></div>`).join(''):'<p class="sub">Discute avec les habitants et découvre leur métier pour les ajouter automatiquement à tes contacts.</p>'}</div>`}
function phoneDialerHTML(){const fire=emergencyResponse?.statusText,pol=policeEmergencyStatus(),samu=samuResponse?.statusText;return `${phoneBackButton()}<div class="phoneDialer card"><div class="sectionKicker">APPELS</div><h3>☎️ Composer un numéro</h3><input id="phoneNumber" class="lifeInput phoneNumber" inputmode="numeric" maxlength="3" placeholder="17 ou 18"><div class="phoneKeypad">${['1','2','3','4','5','6','7','8','9','⌫','0','📞'].map(k=>`<button class="phoneKey" data-key="${k}">${k}</button>`).join('')}</div><p class="sub">17 : police-secours • 18 : sapeurs-pompiers</p>${fire?`<div class="phoneEmergencyLive"><small>🚒 ${fire}</small></div>`:''}${samu?`<div class="phoneEmergencyLive"><small>🚑 ${samu}</small></div>`:''}${pol?`<div class="phoneEmergencyLive"><small>🚔 ${pol}</small></div>`:''}</div>`}
function phoneTransportHTML(){const p=phoneOutdoorPos(),stops=BUS_STOPS.map(s=>({...s,d:Math.hypot(p.x-s.x,p.z-s.z)})).sort((a,b)=>a.d-b.d).slice(0,6),station=stationPosition(),sd=Math.hypot(p.x-station.x,p.z-station.z);return `${phoneBackButton()}<div class="card"><div class="sectionKicker">MOBILITÉS</div><h3>🚌 Transports</h3><div class="grid2"><button id="phoneOpenBusMap" class="menuBtn primary">Plan du réseau</button><button id="phoneOpenWorldMap" class="menuBtn">Région & trains</button></div></div><div class="card"><h3>Arrêts proches</h3>${stops.map(s=>`<div class="phoneTransitRow"><div><b>🚏 ${s.name}</b><small>${Math.round(s.d)} m • ${s.lines.join(' / ')}</small><small>${s.lines.slice(0,2).map(id=>{const l=busLineById(id);return l?`${id}: ${busArrivalSummary(s,l)}`:''}).filter(Boolean).join('<br>')}</small></div><button class="tinyBtn phoneMapPlace" data-x="${s.x}" data-z="${s.z}" data-name="${encodeURIComponent(s.name)}">Carte</button></div>`).join('')}</div><div class="card"><h3>🚆 ${city().station.name}</h3><p class="sub">${Math.round(sd)} m • trains interurbains vers les autres villes.</p><button class="menuBtn phoneMapPlace" data-x="${station.x}" data-z="${station.z}" data-name="${encodeURIComponent(city().station.name)}">Voir sur la carte</button></div>`}
function phoneMapHTML(){return `${phoneBackButton()}<div class="card"><div class="sectionKicker">CARTES</div><h3>🗺️ Où veux-tu regarder ?</h3><div class="phoneListButtons"><button id="phoneLocalMap" class="menuBtn primary">📍 Carte locale</button><button id="phoneTransitMap" class="menuBtn">🚌 Réseau de transports</button><button id="phoneRegionMap" class="menuBtn">🌍 Carte de la région</button></div></div>`}
function phoneAgendaHTML(){return `${phoneBackButton()}${agendaHTML()}`}
function callPhoneNumber(number){const n=String(number||'').replace(/\D/g,'');if(hashStr(`phone:${n}`)===PRIVATE_FIRE_CHEAT_HASH){callFirefighters(true);openSheet('phoneDialer');return}if(n==='18'){callFirefighters(false);openSheet('phoneDialer');return}if(n==='17'){callPoliceEmergency();openSheet('phoneDialer');return}toast('📵 Numéro inconnu. Essaie 17 ou 18.')}
function bindPhonePanel(panel){
 $$('.phoneBack').forEach(b=>b.onclick=phoneBack);$$('[data-phone-app]').forEach(b=>b.onclick=()=>openSheet(b.dataset.phoneApp));
 $$('.phoneMapPlace').forEach(b=>b.onclick=()=>showPhoneMapPoint(b.dataset.x,b.dataset.z,decodeURIComponent(b.dataset.name||'Destination')));
 $$('.quickCall').forEach(b=>b.onclick=()=>callPhoneNumber(b.dataset.number));
 if(panel==='phoneDialer'){$$('.phoneKey').forEach(b=>b.onclick=()=>{const i=$('#phoneNumber');if(!i)return;const k=b.dataset.key;if(k==='⌫')i.value=i.value.slice(0,-1);else if(k==='📞')callPhoneNumber(i.value);else if(i.value.length<3)i.value+=k})}
 $('#phoneLocalMap')?.addEventListener('click',()=>{closeSheet();mapPhoneTarget=null;showLocalMap();$('#mapOverlay').classList.remove('hidden');drawMap()});
 $('#phoneTransitMap')?.addEventListener('click',()=>{closeSheet();mapPhoneTarget=null;showFullBusMap()});
 $('#phoneRegionMap')?.addEventListener('click',()=>{closeSheet();mapPhoneTarget=null;showWorldMap()});
 $('#phoneOpenBusMap')?.addEventListener('click',()=>{closeSheet();mapPhoneTarget=null;showFullBusMap()});
 $('#phoneOpenWorldMap')?.addEventListener('click',()=>{closeSheet();mapPhoneTarget=null;showWorldMap()});
 if(panel==='phoneAgenda')bindAgenda()
}
...
function openSheet(panel){
 ...
 if(panel==='phone'){t.textContent='Téléphone';b.innerHTML=phoneHomeHTML()}
 if(panel==='phoneMap'){t.textContent='Carte';b.innerHTML=phoneMapHTML()}
 if(panel==='phoneAgenda'){t.textContent='Agenda';b.innerHTML=phoneAgendaHTML()}
 if(panel==='phoneGuide'){t.textContent='Guide';b.innerHTML=phoneGuideHTML()}
 if(panel==='phoneDirectory'){t.textContent='Annuaire';b.innerHTML=phoneDirectoryHTML()}
 if(panel==='phoneContacts'){t.textContent='Contacts';b.innerHTML=phoneContactsHTML()}
 if(panel==='phoneDialer'){t.textContent='Téléphone';b.innerHTML=phoneDialerHTML()}
 if(panel==='phoneTransport'){t.textContent='Transports';b.innerHTML=phoneTransportHTML()}
 if(panel==='emergencyConfirm'){t.textContent='Intervention pompiers';b.innerHTML=emergencyConfirmHTML()}
...
function menuHTML(){return `<div class="menuHero"><div><div class="sectionKicker">STREETQUEST V22.9.3</div><h3>${mpNickname()}</h3><p>${city().name} • ${streetCoords()} • ${formatGameTime()}</p></div><button class="avatarMiniBtn" id="menuAvatar">🎨</button></div>
...
function switchCity(id,viaTrain=false){
 clearCityEmergencyServices();clearTarget(false);busWaitRequest=null;busJourneyPlan=null;busRide=null;currentBoardBusId=null;state.cityId=id;activateCityTransit(id);const sp=city().spawn||{x:2,z:8};state.pos={x:sp.x,z:sp.z};state.yaw=0;state.pitch=0;if(!state.visitedCities.includes(id))state.visitedCities.push(id);
 for(const[k]of[...chunks])unload(k);applyCityAtmosphere();initBusFleet();refreshCityLandmark();ensureChunks(true);initCityEmergencyServices();save();closeSheet();for(const rid of [...remotePlayers.keys()])removeRemoteAvatar(rid);if(mpSocket?.connected){setMpStatus(true,1,'Changement de ville…');mpSocket.emit('player:join',{name:mpNickname(),city:state.cityId,x:state.pos.x,z:state.pos.z,yaw:state.yaw,avatar:avatarPayload(),avatarVersion:state.avatarVersion||1});if(voiceEnabled)setTimeout(()=>mpSocket?.emit('voice:state',{enabled:true}),80)}toast(`${viaTrain?'🚆 Arrivée à':'Bienvenue à'} ${city().name}`)
}
function bagHTML(){
 ...
 const rxs=(state.prescriptions||[]).filter(x=>x.location!=='home').map(rx=>prescriptionHTML(rx,false)).join('');const meds=Object.entries(state.medications||{}).filter(([,q])=>q>0).map(([id,q])=>{const m=MEDICINES[id],rx=(state.prescriptions||[]).find(x=>x.active!==false&&x.medicineId===id);return`<div class="item"><div class="itemIcon">${m?.icon||'💊'}</div><div class="itemMain"><b>${m?.name||id}</b><small>${q} dose(s) • ${rx?prescriptionDueText(rx):'sans ordonnance active'}</small></div>${rx?`<button class="menuBtn takeDose" data-rx="${rx.uid}">Prendre</button>`:''}</div>`}).join('');
 return`<div class="card bagSummary"><div><b>${invCount()}/${state.bagMax}</b><small>objets</small></div><div><b>${Math.round(state.hunger)}</b><small>faim</small></div><div><b>${Math.round(state.thirst)}</b><small>soif</small></div><div><b>${Math.round(state.hygiene)}</b><small>hygiène</small></div></div>${healthStatusHTML()}<div class="card"><h3>Équipement</h3>${ws}</div><div class="card"><h3>Uniformes</h3>${uniforms||'<p class="sub">Aucun uniforme dans le sac.</p>'}</div><div class="card"><h3>Ordonnances & médicaments</h3>${rxs||'<p class="sub">Aucune ordonnance dans le sac.</p>'}${meds||''}</div><div class="card"><h3>Courses & nourriture</h3>${food||'<p class="sub">Aucun aliment dans le sac.</p>'}</div><div class="card"><h3>Autres objets</h3>${inv||'<p class="sub">Aucun autre objet.</p>'}</div>`
}
...
function settingsHTML(){return `<div class="card"><div class="sectionKicker">VERSION</div><h3>StreetQuest V22.9.3</h3><button class="menuBtn full" id="forceUpdate">↻ Vérifier les mises à jour</button></div>
...
function bindSheet(panel){
 ...
 if(panel==='bag'){$$('.equip').forEach(b=>b.onclick=()=>{state.equipped=b.dataset.w;weaponRig.visible=b.dataset.w!=='fists';save();openSheet('bag')});$$('.useMed').forEach(b=>b.onclick=useMed);$$('.useConsumable').forEach(b=>b.onclick=()=>useConsumable(b.dataset.id));$$('.discardFoodBag').forEach(b=>b.onclick=()=>discardFood(b.dataset.uid,'bag'));$$('.eatFoodLot').forEach(b=>b.onclick=()=>consumeFoodLot(b.dataset.uid,false));$$('.eatSpoiledFood').forEach(b=>b.onclick=()=>consumeFoodLot(b.dataset.uid,true));$$('.takeDose').forEach(b=>b.onclick=()=>takePrescriptionDose(b.dataset.rx));$$('.movePrescriptionHome').forEach(b=>b.onclick=()=>movePrescription(b.dataset.id,'home'));$('.callFirefighters')?.addEventListener('click',callFirefighters);$('.wearUniform')?.addEventListener('click',wearWorkUniform);$('.removeUniform')?.addEventListener('click',removeWorkUniform)}
 if(panel==='agenda')bindAgenda();
 if(['phone','phoneMap','phoneAgenda','phoneGuide','phoneDirectory','phoneContacts','phoneDialer','phoneTransport'].includes(panel))bindPhonePanel(panel);
 if(panel==='emergencyConfirm'){$('.fireCareAccept')?.addEventListener('click',acceptFireCare);$('.fireCareDecline')?.addEventListener('click',declineFireCare)}
...
function v20CivilianVisible(n,hour){
 ...
 const presence={home:.07,commute:.98,lunch:.82,leisure:.86,work:o.publicFacing?.48:.24,nightWork:.48}[phase]??.55;
 return roll<clamp(presence*v20CrowdFactor(hour)*1.15,.045,1)
}
...
$('#navMap')?.addEventListener('click',showFullBusMap);$$('.nav[data-panel]').forEach(b=>b.onclick=()=>openSheet(b.dataset.panel));
