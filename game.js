...
function busTrafficFactor(b,seg){
 ...
 for(const u of allServiceVehicleUnits()){const us=u.currentSeg;if(!us||us.mode!==mode||us.dir!==dir||!u.vehicle?.parent)continue;const laneGap=mode==='h'?Math.abs(u.vehicle.position.z-b.group.position.z):Math.abs(u.vehicle.position.x-b.group.position.x);if(laneGap>1.3)continue;const delta=mode==='h'?(u.vehicle.position.x-b.group.position.x)*dir:(u.vehicle.position.z-b.group.position.z)*dir;if(delta>0&&delta<best)best=delta}
...
function blockedByPerson(x,z){
 ...
 for(const u of allServiceVehicleUnits())if(u.vehicle?.parent&&Math.hypot(x-u.vehicle.position.x,z-u.vehicle.position.z)<1.15)return true;
 return false
}
...
function personDynamicBlocked(n,x,z,pad=.28){
 ...
 for(const u of allServiceVehicleUnits())if(u.vehicle?.parent&&Math.hypot(x-u.vehicle.position.x,z-u.vehicle.position.z)<1.45)return true;
 return false
}
...
function updateCamera(t=0){
 const rescue=emergencyResponse?.patientOnBoard&&emergencyResponse.vehicle,riding=busRide&&busVehicles.find(x=>x.id===busRide.vehicleId),bob=!riding&&!rescue&&(Math.abs(moveStick.x)+Math.abs(moveStick.y)>.15)?Math.sin(t*.012)*.022:0;
 if(rescue){rescue.updateMatrixWorld(true);const local=rescue.localToWorld(new THREE.Vector3(.32,1.28,.35));camera.position.copy(local);const a=-rescue.rotation.y,c=Math.cos(a),si=Math.sin(a);camera.lookAt(local.x+si,local.y-.03,local.z-c);return}
 if(riding){
...
function intersectionClearFor(c,ix,iz){
 ...
 for(const u of allServiceVehicleUnits()){const seg=u.currentSeg;if(!seg||!u.vehicle?.parent)continue;const x=u.vehicle.position.x,z=u.vehicle.position.z;if(x>ix-1.2&&x<ix+12.2&&z>iz-1.2&&z<iz+12.2&&seg.mode!==c.mode)return false}
 return true
}
...
function emergencyRoute(a,b){const sa=nearestRoadPoint(a),sb=nearestRoadPoint(b),vx=Math.round((sa.x-5.5)/CHUNK)*CHUNK+5.5,hz=Math.round((sb.z-5.5)/CHUNK)*CHUNK+5.5;const pts=[{x:a.x,z:a.z,offRoad:true},{x:sa.x,z:sa.z,offRoad:true},{x:vx,z:sa.z,road:true},{x:vx,z:hz,road:true},{x:sb.x,z:sb.z,road:true},{x:b.x,z:b.z,offRoad:true}];return pts.filter((p,i)=>!i||Math.hypot(p.x-pts[i-1].x,p.z-pts[i-1].z)>.3)}
...
function makeFireResponseVehicle(){const g=createCarVisual(0xb82f2d,'van');makeEmergencyLightBar(g,1.88);const white=new THREE.MeshStandardMaterial({color:0xf0e9df,roughness:.65});const stripe=new THREE.Mesh(new THREE.BoxGeometry(2.02,.20,3.25),white);stripe.position.set(0,.73,.12);g.add(stripe);const sign=makeSign('SAPEURS-POMPIERS','#ffffff');sign.scale.set(1.45,.28,.3);sign.position.set(0,1.38,2.17);g.add(sign);const bed=new THREE.Mesh(new THREE.BoxGeometry(.72,.12,1.55),new THREE.MeshStandardMaterial({color:0xdedede,roughness:.8}));bed.position.set(.38,.92,.25);g.add(bed);return g}
function makeRescueVehicle(){return makeFireResponseVehicle()}
function makeSamuVehicle(){const g=createCarVisual(0xf0f1ef,'van');makeEmergencyLightBar(g,1.88);const blue=new THREE.MeshStandardMaterial({color:0x285b8c,roughness:.55});for(const z of [-1.35,.2,1.45]){const s=new THREE.Mesh(new THREE.BoxGeometry(2.03,.16,.40),blue);s.position.set(0,.75,z);g.add(s)}const sign=makeSign('SAMU','#164f82');sign.scale.set(1.2,.30,.3);sign.position.set(0,1.40,2.17);g.add(sign);const cross=makeSign('✚','#d93f45');cross.scale.set(.40,.26,.3);cross.position.set(.62,1.43,-2.14);cross.rotation.y=Math.PI;g.add(cross);return g}
function makePoliceResponseVehicle(){const g=createCarVisual(0x244d78,'sedan');makeEmergencyLightBar(g,1.56);const sign=makeSign('POLICE','#d8efff');sign.scale.set(1.15,.28,.3);sign.position.set(0,1.18,2.03);g.add(sign);return g}
...
function moveResponderCrewToward(list,target,dt,speed=1.35){for(let i=0;i<list.length;i++){const n=list[i],tx=target.x+(i-((list.length-1)/2))*.55,tz=target.z+.35,dx=tx-n.group.position.x,dz=tz-n.group.position.z,d=Math.hypot(dx,dz);if(d>.15){const k=Math.min(d,speed*dt);n.group.position.x+=dx/d*k;n.group.position.z+=dz/d*k;setHeading(n,dx,dz)}}}
...
function serviceTrafficAhead(unit,seg,max=18){let best=max;const px=unit.vehicle.position.x,pz=unit.vehicle.position.z;const check=(x,z,mode,dir)=>{if(mode!==seg.mode||dir!==seg.dir)return;const laneGap=seg.mode==='h'?Math.abs(z-pz):Math.abs(x-px);if(laneGap>1.25)return;const delta=seg.mode==='h'?(x-px)*seg.dir:(z-pz)*seg.dir;if(delta>0&&delta<best)best=delta};for(const c of cars)if(c?.group?.parent)check(c.group.position.x,c.group.position.z,c.mode,c.dir);for(const b of busVehicles){const bs=busSegment(b);if(bs&&b.group?.parent)check(b.group.position.x,b.group.position.z,bs.mode,bs.dir)}for(const o of allServiceVehicleUnits()){if(o===unit||!o.vehicle?.parent||!o.currentSeg)continue;check(o.vehicle.position.x,o.vehicle.position.z,o.currentSeg.mode,o.currentSeg.dir)}return best}
...
function createDispatchUnit(type,res,target,opts={}){let vehicle,station;if(res.mutualAid){vehicle=serviceMutualAidSpawn(res,type);station={name:`Renfort ${CITIES.find(c=>c.id===res.originCity)?.name||res.originCity}`,x:vehicle.position.x,z:vehicle.position.z}}else{const v=localServiceSpawn(res,type);vehicle=v.vehicle;station=v.station}const stop=nearestRoadPoint(target),route=emergencyRoute(vehicle.position,stop);return{type,token:res.token,originCity:res.originCity,mutualAid:res.mutualAid,preDelay:res.delay||0,vehicle,station,route,index:1,baseSpeed:opts.speed||9,currentSpeed:0,emergency:opts.emergency!==false,target:{...target},stopPoint:stop,patientOnBoard:false,crew:[],statusText:'',phase:'toCall',dwell:0}}
...
function removeEmergencyVehicle(){if(emergencyResponse?.vehicle?.parent)scene.remove(emergencyResponse.vehicle);removeResponderCrew(emergencyResponse?.crew||[]);releaseServiceToken(emergencyResponse?.token);emergencyResponse=null;if(samuResponse?.vehicle?.parent)scene.remove(samuResponse.vehicle);removeResponderCrew(samuResponse?.crew||[]);releaseServiceToken(samuResponse?.token);samuResponse=null}
function startFireTransport(e,urgent=false){removeResponderCrew(e.crew);e.patientOnBoard=true;e.phase='toHospital';e.emergency=urgent;e.hospital=nearestHospital(e.vehicle.position);if(!e.hospital){toast('Aucun hôpital disponible.');removeEmergencyVehicle();return}e.route=emergencyRoute(e.vehicle.position,nearestRoadPoint(e.hospital));e.index=1;e.statusText=`Transport ${urgent?'urgent ':''}vers ${e.hospital.name}`;toast(`🚑 ${urgent?'Transport urgent':'Transport'} vers ${e.hospital.name}.`)}
...
function finishHospitalArrival(hospital,via='fire'){if(emergencyResponse?.vehicle?.parent)scene.remove(emergencyResponse.vehicle);removeResponderCrew(emergencyResponse?.crew||[]);releaseServiceToken(emergencyResponse?.token);emergencyResponse=null;if(samuResponse?.vehicle?.parent)scene.remove(samuResponse.vehicle);removeResponderCrew(samuResponse?.crew||[]);releaseServiceToken(samuResponse?.token);samuResponse=null;state.hospitalFlow={admitted:true,step:'arrival',viaFirefighters:via==='fire',viaSamu:via==='samu'};enterInterior('shop',{type:'clinic',displayName:hospital.name,name:hospital.name},{preserveReturn:false});state.pos={x:0,z:5.5};toast(`🏥 Arrivée à ${hospital.name}. L’équipe médicale prend le relais.`);openSheet('physicalShop')}
function updateEmergencyResponse(dt){const e=emergencyResponse;if(!e)return;if(e.phase==='queued')return;const t=performance.now();if(e.phase==='toPatient')refreshServiceStatus(e,'VSAV en route');if(e.patientOnBoard){state.pos.x=e.vehicle.position.x;state.pos.z=e.vehicle.position.z}if(e.phase==='toPatient'||e.phase==='toHospital'){const done=serviceDrive(e,dt,t);if(!done)return;if(e.phase==='toPatient'){e.emergency=false;e.phase='assess';e.careActive=true;e.dwell=e.forceTest&&!state.disease?6:5;e.crew=[spawnResponder('firefighter',e.vehicle.position.x-.7,e.vehicle.position.z+.5,'Chef d’agrès'),spawnResponder('firefighter',e.vehicle.position.x,e.vehicle.position.z+.8,'Sapeur-pompier'),spawnResponder('firefighter',e.vehicle.position.x+.7,e.vehicle.position.z+.5,'Sapeur-pompier')];e.statusText='Pompiers sur place • bilan et prise en charge';toast('🚒 Les pompiers sont arrivés physiquement et commencent le bilan.');return}if(e.phase==='toHospital'){finishHospitalArrival(e.hospital,'fire');return}}if(e.phase==='assess'||e.phase==='waitingSamu'){moveResponderCrewToward(e.crew,e.patient,dt);if(e.phase==='waitingSamu')return;e.dwell-=dt;if(e.dwell>0)return;if(e.forceTest&&!state.disease){e.statusText='Intervention de test terminée';toast('🧪 Intervention 18 de test terminée.');removeEmergencyVehicle();return}const sev=state.disease?.severity||0;e.careActive=true;if(sev>=82){requestSamuFromFire(e);return}startFireTransport(e,sev>=72||state.hp<55)}}
function updateSamuResponse(dt){const s=samuResponse;if(!s)return;const t=performance.now();if(s.phase==='toPatient')refreshServiceStatus(s,'SAMU en route');if(s.patientOnBoard){state.pos.x=s.vehicle.position.x;state.pos.z=s.vehicle.position.z}if(s.phase==='toPatient'||s.phase==='toHospital'||s.phase==='returning'){const done=serviceDrive(s,dt,t);if(!done)return;if(s.phase==='toPatient'){s.emergency=false;s.phase='stabilize';s.dwell=6;s.crew=[spawnResponder('doctor',s.vehicle.position.x-.75,s.vehicle.position.z+.55,'Médecin SAMU'),spawnResponder('nurse',s.vehicle.position.x,s.vehicle.position.z+.8,'Infirmier SAMU'),spawnResponder('ambulance',s.vehicle.position.x+.75,s.vehicle.position.z+.55,'Ambulancier')];s.careActive=true;s.statusText='SAMU sur place • médicalisation en cours';toast('🚑 Le SAMU arrive : médecin, infirmier et ambulancier prennent le relais.');return}if(s.phase==='toHospital'){finishHospitalArrival(s.hospital,'samu');return}if(s.phase==='returning'){if(s.vehicle?.parent)scene.remove(s.vehicle);removeResponderCrew(s.crew);releaseServiceToken(s.token);samuResponse=null;return}}if(s.phase==='stabilize'){moveResponderCrewToward(s.crew,s.target,dt,1.45);s.dwell-=dt;if(s.dwell>0)return;if(state.disease){state.disease.severity=clamp((state.disease.severity||0)-12,0,100);state.hp=clamp(state.hp+6,0,state.maxHp)}const critical=(state.disease?.severity||0)>=86||state.hp<45;if(critical){removeResponderCrew(s.crew);const f=s.fire;if(f){removeResponderCrew(f.crew);if(f.vehicle?.parent)scene.remove(f.vehicle);releaseServiceToken(f.token);emergencyResponse=null}s.patientOnBoard=true;s.phase='toHospital';s.emergency=true;s.hospital=nearestHospital(s.vehicle.position);if(!s.hospital){toast('🚑 Aucun hôpital disponible pour le transport médicalisé.');s.patientOnBoard=false;s.phase='returning';s.emergency=false;const dest=s.mutualAid?walkingEntryPosition(city(),mutualAidEntrySide(s.originCity,state.cityId)||'west',0):nearestRoadPoint(s.station);s.route=emergencyRoute(s.vehicle.position,dest);s.index=1;s.statusText='SAMU disponible • retour à sa base';return}s.route=emergencyRoute(s.vehicle.position,nearestRoadPoint(s.hospital));s.index=1;s.statusText=`Patient stabilisé • transport médicalisé urgent vers ${s.hospital.name}`;toast('🚑 Le SAMU décide un transport médicalisé urgent.')}else{const f=s.fire;toast('🩺 État stabilisé par le SAMU. Les pompiers assurent le transport.');removeResponderCrew(s.crew);if(f)startFireTransport(f,(state.disease?.severity||0)>=70);s.phase='returning';s.emergency=false;const dest=s.mutualAid?walkingEntryPosition(city(),mutualAidEntrySide(s.originCity,state.cityId)||'west',0):nearestRoadPoint(s.station);s.route=emergencyRoute(s.vehicle.position,dest);s.index=1;s.statusText='SAMU disponible • retour à sa base'}}}
...
function menuHTML(){return `<div class="menuHero"><div><div class="sectionKicker">STREETQUEST V22.9</div><h3>${mpNickname()}</h3><p>${city().name} • ${streetCoords()} • ${formatGameTime()}</p></div><button class="avatarMiniBtn" id="menuAvatar">🎨</button></div>
...
function settingsHTML(){return `<div class="card"><div class="sectionKicker">VERSION</div><h3>StreetQuest V22.9</h3><button class="menuBtn full" id="forceUpdate">↻ Vérifier les mises à jour</button></div>
