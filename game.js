...
function makeRescueVehicle(){const g=createCarVisual(0xb93431,'van');const bar=new THREE.Mesh(new THREE.BoxGeometry(.95,.12,.30),new THREE.MeshBasicMaterial({color:0x5aa8ff}));bar.position.set(0,1.82,-.1);g.add(bar);const bed=new THREE.Mesh(new THREE.BoxGeometry(.75,.13,1.65),new THREE.MeshStandardMaterial({color:0xdedede,roughness:.8}));bed.position.set(.38,.92,.25);g.add(bed);const cross=makeSign('✚','#ffffff');cross.scale.set(.42,.28,.3);cross.position.set(0,1.22,2.14);g.add(cross);return g}
function callFirefighters(){
 ...
 const station=nearestCivic('fireStation'),hospital=nearestHospital();if(!station||!hospital)return toast('Aucun service de secours ou hôpital disponible dans cette ville.');const patient={...state.pos},route=emergencyRoute(station,patient),g=makeRescueVehicle();g.position.set(route[0].x,0,route[0].z);scene.add(g);const eta=Math.min(9.5,Math.max(1.5,routeLength(route)/9));emergencyResponse={vehicle:g,phase:'toPatient',route,index:1,speed:9.2,patient,hospital,patientOnBoard:false,dwell:0,statusText:`VSAV en route • ETA théorique ${eta.toFixed(1)} min`};toast(`🚒 Secours déclenchés • arrivée théorique ${eta.toFixed(1)} min`)
}
...
function updateEmergencyResponse(dt){const e=emergencyResponse;if(!e?.vehicle)return;if(e.dwell>0){e.dwell-=dt;if(e.dwell<=0&&e.phase==='pickup'){e.patientOnBoard=true;e.phase='toHospital';e.route=emergencyRoute(e.vehicle.position,e.hospital);e.index=1;e.statusText=`Pris en charge • transport vers ${e.hospital.name}`;toast('🚑 Tu es installé sur le brancard. Direction l’hôpital le plus proche.')}return}
...
function menuHTML(){return `<div class="menuHero"><div><div class="sectionKicker">STREETQUEST V22.7</div><h3>${mpNickname()}</h3><p>${city().name} • ${streetCoords()} • ${formatGameTime()}</p></div><button class="avatarMiniBtn" id="menuAvatar">🎨</button></div>
...
function settingsHTML(){return `<div class="card"><div class="sectionKicker">VERSION</div><h3>StreetQuest V22.7</h3><button class="menuBtn full" id="forceUpdate">↻ Vérifier les mises à jour</button></div>
