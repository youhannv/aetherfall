...
function tryMoveCar(c,dx,dz){const nx=c.group.position.x+dx,nz=c.group.position.z+dz;for(const u of allServiceVehicleUnits()){if(!u.vehicle?.parent)continue;if(vehicleOccupiesPoint(u.vehicle,nx,nz,.48))return false}c.group.position.x=nx;c.group.position.z=nz;return true}
...
function emergencyParkingPoint(target,type='fire'){
 const bx=Math.floor(target.x/CHUNK)*CHUNK,bz=Math.floor(target.z/CHUNK)*CHUNK,c=[];
 for(const base of [bx,bx+CHUNK]){c.push({x:base+3.2,z:target.z-4.2,mode:'v',dir:1,road:true});c.push({x:base+7.8,z:target.z+4.2,mode:'v',dir:-1,road:true})}
 for(const base of [bz,bz+CHUNK]){c.push({x:target.x-4.2,z:base+7.8,mode:'h',dir:1,road:true});c.push({x:target.x+4.2,z:base+3.2,mode:'h',dir:-1,road:true})}
 c.sort((a,b)=>Math.hypot(a.x-target.x,a.z-target.z)-Math.hypot(b.x-target.x,b.z-target.z));return c[0]
}
...
function inferParkedSeg(stop){return stop?.mode?{mode:stop.mode,dir:stop.dir||1,laneCoord:stop.mode==='h'?stop.z:stop.x,roadBase:stop.mode==='h'?Math.floor(stop.z/CHUNK)*CHUNK:Math.floor(stop.x/CHUNK)*CHUNK,offRoad:false}:null}
function parkServiceUnit(unit){unit.currentSpeed=0;unit.currentSeg=null;unit.parkedSeg=inferParkedSeg(unit.stopPoint);unit.vehicle.userData.serviceParked=true;unit.vehicle.userData.serviceType=unit.type}
function makeEmergencyUtilityBase(color=0xffffff){
 const g=new THREE.Group(),body=new THREE.MeshStandardMaterial({color,roughness:.60,metalness:.08}),dark=new THREE.MeshStandardMaterial({color:0x20262b,roughness:.45}),glass=new THREE.MeshStandardMaterial({color:0x385a70,roughness:.24,metalness:.10});
 const lower=new THREE.Mesh(new THREE.BoxGeometry(2.12,1.05,5.45),body);lower.position.y=.70;g.add(lower);const box=new THREE.Mesh(new THREE.BoxGeometry(2.04,1.55,3.70),body);box.position.set(0,1.72,.48);g.add(box);const cab=new THREE.Mesh(new THREE.BoxGeometry(2.02,1.22,1.62),body);cab.position.set(0,1.32,-1.98);g.add(cab);const wind=new THREE.Mesh(new THREE.BoxGeometry(1.68,.64,.06),glass);wind.position.set(0,1.58,-2.80);g.add(wind);
 for(const sx of [-1,1])for(const z of [-1.84,1.78]){const wh=new THREE.Mesh(new THREE.CylinderGeometry(.43,.43,.22,12),dark);wh.rotation.z=Math.PI/2;wh.position.set(sx*1.08,.43,z);g.add(wh)}const bumper=new THREE.Mesh(new THREE.BoxGeometry(2.05,.22,.18),dark);bumper.position.set(0,.48,-2.80);g.add(bumper);g.userData.vehicleLength=5.65;g.userData.vehicleWidth=2.18;g.userData.vehicleHeight=2.55;return g
}
...
function spawnCrewOutsideVehicle(vehicle,roles){const length=vehicle.userData.vehicleLength||5.4,spots=[[-1.55,length*.23],[1.55,length*.23],[-1.45,-length*.20],[1.45,-length*.20],[0,length*.58]];return roles.map((r,i)=>{const p=vehicleWorldPoint(vehicle,spots[i][0],spots[i][1]);const n=spawnResponder(r.role,p.x,p.z,r.label);n.group.rotation.y=vehicle.rotation.y;return n})}
...
function acceptFireCare(){const e=emergencyResponse;if(!e||e.phase!=='confirm')return closeSheet();e.phase='assess';e.careActive=true;e.dwell=e.forceTest&&!state.disease?5.5:7.5;e.careElapsed=0;ensureFireStretcher(e);e.statusText='Pompiers sur place • bilan en cours';closeSheet();toast('🚒 « Très bien. On va d’abord faire un bilan avec vous. »')}
...
function beginFireLoading(e,urgent=false){e.transportUrgent=urgent;e.phase='loading';e.loadingElapsed=0;e.loadingStart={x:state.pos.x,z:state.pos.z};e.careActive=true;e.hospital=nearestHospital(e.vehicle.position);if(!e.hospital){toast('Aucun hôpital disponible.');declineFireCare();return}ensureFireStretcher(e);e.statusText='Installation sur le brancard • préparation du transport';toast('🚑 L’équipe installe le brancard et prépare le transport.')}
function animateFireLoading(e,dt,t){const st=ensureFireStretcher(e);e.loadingElapsed=(e.loadingElapsed||0)+dt;const dur=3.6,p=Math.min(1,e.loadingElapsed/dur),rear=vehicleWorldPoint(e.vehicle,0,(e.vehicle.userData.vehicleLength||5.4)*.54),start=e.loadingStart||{x:state.pos.x,z:state.pos.z};const sx=start.x+(rear.x-start.x)*p,sz=start.z+(rear.z-start.z)*p;st.position.x=sx;st.position.z=sz;st.rotation.y=e.vehicle.rotation.y;state.pos.x=sx;state.pos.z=sz;animateCrewCare(e.crew,{x:sx,z:sz},dt,t);if(p<1)return false;removeResponderCrew(e.crew);removeServiceStretcher(e);e.patientOnBoard=true;e.careActive=false;e.phase='toHospital';e.emergency=!!e.transportUrgent;e.stopPoint=nearestRoadPoint(e.hospital);e.route=emergencyRoute(e.vehicle.position,e.stopPoint);e.index=1;e.parkedSeg=null;e.vehicle.userData.serviceParked=false;e.statusText=`Transport ${e.emergency?'urgent ':''}vers ${e.hospital.name}`;toast(`🚑 Départ ${e.emergency?'en urgence ':''}vers ${e.hospital.name}.`);return true}
...
function makeFireResponseVehicle(){const g=makeEmergencyUtilityBase(0xc7332f);makeEmergencyLightBar(g,2.62);const white=new THREE.MeshStandardMaterial({color:0xf3eee7,roughness:.58});for(const z of [-1.35,.15,1.50]){const stripe=new THREE.Mesh(new THREE.BoxGeometry(2.16,.18,.58),white);stripe.position.set(0,.88,z);g.add(stripe)}const sign=makeSign('VSAV  SAPEURS-POMPIERS','#ffffff');sign.scale.set(1.35,.26,.28);sign.position.set(0,1.86,2.78);g.add(sign);const side=makeSign('18  SECOURS','#ffffff');side.scale.set(.82,.22,.28);side.position.set(0,1.52,-2.84);side.rotation.y=Math.PI;g.add(side);return g}
...
function updateEmergencyResponse(dt){const e=emergencyResponse;if(!e)return;if(e.phase==='queued')return;const t=performance.now();if(e.phase==='toPatient')refreshServiceStatus(e,'VSAV en route');if(e.patientOnBoard){state.pos.x=e.vehicle.position.x;state.pos.z=e.vehicle.position.z}if(e.phase==='toPatient'||e.phase==='toHospital'||e.phase==='returning'){const done=serviceDrive(e,dt,t);if(!done)return;if(e.phase==='toPatient'){parkServiceUnit(e);e.emergency=false;e.phase='confirm';e.careActive=false;e.crew=spawnCrewOutsideVehicle(e.vehicle,[{role:'firefighter',label:'Chef d’agrès'},{role:'firefighter',label:'Sapeur-pompier'},{role:'firefighter',label:'Sapeur-pompier'}]);e.statusText='Pompiers sur place • parle au chef d’agrès pour confirmer';toast('🚒 Les pompiers sont arrivés. Approche-toi du chef d’agrès pour confirmer ou refuser l’intervention.');return}if(e.phase==='toHospital'){finishHospitalArrival(e.hospital,'fire');return}if(e.phase==='returning'){removeServiceStretcher(e);if(e.vehicle?.parent)scene.remove(e.vehicle);removeResponderCrew(e.crew);releaseServiceToken(e.token);emergencyResponse=null;return}}if(e.phase==='confirm'){moveResponderCrewToward(e.crew,e.patient,dt,1.25);return}if(e.phase==='loading'){animateFireLoading(e,dt,t);return}if(e.phase==='assess'||e.phase==='waitingSamu'){if(e.phase==='waitingSamu'){animateCrewCare(e.crew,e.patient,dt,t);return}e.careElapsed=(e.careElapsed||0)+dt;animateCrewCare(e.crew,e.patient,dt,t);const st=ensureFireStretcher(e),q=Math.min(1,e.careElapsed/3.2),rear=e.stretcherStart||{x:st.position.x,z:st.position.z};st.position.x=rear.x+(e.patient.x-rear.x)*q;st.position.z=rear.z+(e.patient.z-rear.z)*q;e.dwell-=dt;if(e.dwell>0)return;if(e.forceTest&&!state.disease){e.statusText='Bilan terminé • aucune évacuation nécessaire';toast('🚒 « Le bilan est rassurant, aucune évacuation n’est nécessaire. »');removeResponderCrew(e.crew);removeServiceStretcher(e);e.phase='returning';e.careActive=false;e.stopPoint=nearestRoadPoint(e.station);e.route=emergencyRoute(e.vehicle.position,e.stopPoint);e.index=1;e.parkedSeg=null;e.vehicle.userData.serviceParked=false;return}const sev=state.disease?.severity||0;e.careActive=true;if(sev>=82){requestSamuFromFire(e);return}startFireTransport(e,sev>=72||state.hp<55)}}
...
function menuHTML(){return `<div class="menuHero"><div><div class="sectionKicker">STREETQUEST V22.9.2</div><h3>${mpNickname()}</h3><p>${city().name} • ${streetCoords()} • ${formatGameTime()}</p></div><button class="avatarMiniBtn" id="menuAvatar">🎨</button></div>
...
function settingsHTML(){return `<div class="card"><div class="sectionKicker">VERSION</div><h3>StreetQuest V22.9.2</h3><button class="menuBtn full" id="forceUpdate">↻ Vérifier les mises à jour</button></div>
