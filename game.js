...
const base={
 ...
 landOwned:false,housingStage:0,homeLevel:1,homeBank:0,homeStorage:{medkit:0},homeStock:[],homePlaced:[],reputation:0,restCount:0,artifactBag:[],discoveredShops:[],hunger:70,thirst:70,hygiene:60,worldLayoutVersion:229,trainTrips:0,visitedCities:['paris'],
...
function loadState(){
 try{
   ...
   loaded.worldLayoutVersion=229;loaded.trainTrips=raw.trainTrips||0;loaded.visitedCities=raw.visitedCities||[loaded.cityId||'paris'];
...
async function init(){
 ...
 createAtmosphere();activateCityTransit(state.cityId);applyCityAtmosphere();initBusFleet();refreshCityLandmark();initCityEmergencyServices();setupWorldTap();setupDesktopControls();setupMapUI();
...
function serviceRoadSegment(unit){const raw=unit.route?.[unit.index];if(!raw)return null;const pos=unit.vehicle.position,dx0=raw.x-pos.x,dz0=raw.z-pos.z,dist0=Math.hypot(dx0,dz0);if(raw.offRoad||dist0<.001)return{target:raw,dx:dx0,dz:dz0,dist:dist0,offRoad:true,mode:Math.abs(dx0)>=Math.abs(dz0)?'h':'v',dir:Math.abs(dx0)>=Math.abs(dz0)?(dx0>=0?1:-1):(dz0>=0?1:-1),laneCoord:null};const mode=Math.abs(dx0)>=Math.abs(dz0)?'h':'v',dir=mode==='h'?(dx0>=0?1:-1):(dz0>=0?1:-1);let tx=raw.x,tz=raw.z,laneCoord,roadBase;if(mode==='h'){roadBase=Math.round((raw.z-5.5)/CHUNK)*CHUNK;laneCoord=roadBase+(dir>0?7.8:3.2);tz=laneCoord}else{roadBase=Math.round((raw.x-5.5)/CHUNK)*CHUNK;laneCoord=roadBase+(dir>0?3.2:7.8);tx=laneCoord}let target={...raw,x:tx,z:tz};const dx=tx-pos.x,dz=tz-pos.z;return{target,raw,dx,dz,dist:Math.hypot(dx,dz),mode,dir,laneCoord,roadBase,offRoad:false}}
...
function serviceDrive(unit,dt,t){if(!unit?.vehicle||!unit.route?.length)return false;if(unit.preDelay>0){unit.preDelay=Math.max(0,unit.preDelay-dt);flashEmergencyLights(unit.vehicle,t,false);unit.vehicle.visible=false;if(unit.preDelay<=0)unit.vehicle.visible=true;return false}unit.vehicle.visible=true;let seg=serviceRoadSegment(unit);unit.currentSeg=seg;if(!seg)return true;if(seg.dist<.30){unit.vehicle.position.set(seg.target.x,0,seg.target.z);unit.index++;seg=serviceRoadSegment(unit);unit.currentSeg=seg;if(!seg)return true}const priority=!!unit.emergency;flashEmergencyLights(unit.vehicle,t,priority);let laneTarget=seg.laneCoord;const ahead=serviceTrafficAhead(unit,seg,20);let trafficFactor=ahead<5.8?0:ahead<10?(ahead-5.8)/4.2:1;let overtaking=false;if(priority&&ahead<9&&alternateLaneSafe(unit,seg)){overtaking=true;laneTarget=seg.mode==='h'?seg.roadBase+(seg.dir>0?3.2:7.8):seg.roadBase+(seg.dir>0?7.8:3.2);trafficFactor=.82}const signal=serviceIntersectionState(unit,seg,t,priority);let targetSpeed=(unit.baseSpeed||7.5)*(seg.offRoad?.45:1)*trafficFactor;if(signal.mustStop)targetSpeed=0;if(signal.crossSlow)targetSpeed=Math.min(targetSpeed,2.4);const pd=Math.hypot(state.pos.x-unit.vehicle.position.x,state.pos.z-unit.vehicle.position.z);if(!unit.patientOnBoard&&pd<3.2)targetSpeed=Math.min(targetSpeed,1.6);unit.currentSpeed=(unit.currentSpeed||0)+(targetSpeed-(unit.currentSpeed||0))*Math.min(1,dt*(targetSpeed<(unit.currentSpeed||0)?6.8:2.4));if(signal.mustStop&&signal.distance<.45)unit.currentSpeed=0;let dx=seg.dx,dz=seg.dz;if(!seg.offRoad&&laneTarget!=null){if(seg.mode==='h')dz=laneTarget-unit.vehicle.position.z;else dx=laneTarget-unit.vehicle.position.x}const targetYaw=Math.atan2(-dx,-dz),angle=Math.atan2(Math.sin(targetYaw-unit.vehicle.rotation.y),Math.cos(targetYaw-unit.vehicle.rotation.y)),yawRate=priority?1.35:.88;unit.vehicle.rotation.y+=clamp(angle,-yawRate*dt,yawRate*dt);const headingError=Math.abs(angle),headFactor=headingError>.8?.25:headingError>.42?.58:1;let step=Math.min(seg.dist,unit.currentSpeed*headFactor*dt);if(signal.mustStop&&signal.distance>=0)step=Math.min(step,Math.max(0,signal.distance-.32));if(step>.001){const fx=-Math.sin(unit.vehicle.rotation.y),fz=-Math.cos(unit.vehicle.rotation.y);unit.vehicle.position.x+=fx*step;unit.vehicle.position.z+=fz*step;if(!seg.offRoad&&laneTarget!=null&&headingError<.18){const k=Math.min(1,dt*(overtaking?3.2:2.0));if(seg.mode==='h')unit.vehicle.position.z+=(laneTarget-unit.vehicle.position.z)*k;else unit.vehicle.position.x+=(laneTarget-unit.vehicle.position.x)*k}}return false}
...
function phoneDialerHTML(){const fire=emergencyResponse?.statusText,pol=policeEmergencyStatus(),samu=samuResponse?.statusText;return `${phoneBackButton()}<div class="phoneDialer card"><div class="sectionKicker">APPELS</div><h3>☎️ Composer un numéro</h3><input id="phoneNumber" class="lifeInput phoneNumber" inputmode="numeric" maxlength="3" placeholder="17 ou 18"><div class="phoneKeypad">${['1','2','3','4','5','6','7','8','9','⌫','0','📞'].map(k=>`<button class="phoneKey" data-key="${k}">${k}</button>`).join('')}</div><p class="sub">17 : police-secours • 18 : sapeurs-pompiers</p><div class="phoneServiceCapacity"><small>🚒 ${serviceStatusLine('fire')}</small><small>🚑 ${serviceStatusLine('samu')}</small><small>🚔 Patrouilles visibles : ${policePatrols.filter(p=>!p.busy).length}/${policePatrols.length}</small></div>${fire?`<div class="phoneEmergencyLive"><small>🚒 ${fire}</small></div>`:''}${samu?`<div class="phoneEmergencyLive"><small>🚑 ${samu}</small></div>`:''}${pol?`<div class="phoneEmergencyLive"><small>🚔 ${pol}</small></div>`:''}</div>`}
