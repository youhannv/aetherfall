...
const base={
 ...
 landOwned:false,housingStage:0,homeLevel:1,homeBank:0,homeStorage:{medkit:0},homeStock:[],homePlaced:[],reputation:0,restCount:0,artifactBag:[],discoveredShops:[],hunger:70,thirst:70,hygiene:60,worldLayoutVersion:227,trainTrips:0,visitedCities:['paris'],
...
function loadState(){
 try{
   ...
   loaded.worldLayoutVersion=227;loaded.trainTrips=raw.trainTrips||0;loaded.visitedCities=raw.visitedCities||[loaded.cityId||'paris'];
...
function walkCityTransition(link,exitSide,offset=0){
 const dest=CITIES.find(c=>c.id===link.to);if(!dest)return false;removeEmergencyVehicle();clearPoliceResponse();clearTarget(false);busWaitRequest=null;busJourneyPlan=null;busRide=null;currentBoardBusId=null;
...
function nearestRoadPoint(p){const vx=Math.round((p.x-5.5)/CHUNK)*CHUNK+5.5,hz=Math.round((p.z-5.5)/CHUNK)*CHUNK+5.5;return Math.abs(p.x-vx)<Math.abs(p.z-hz)?{x:vx,z:p.z}:{x:p.x,z:hz}}
function emergencyRoute(a,b){const sa=nearestRoadPoint(a),sb=nearestRoadPoint(b),vx=Math.round((sa.x-5.5)/CHUNK)*CHUNK+5.5,hz=Math.round((sb.z-5.5)/CHUNK)*CHUNK+5.5;const pts=[{x:a.x,z:a.z},{x:sa.x,z:sa.z},{x:vx,z:sa.z},{x:vx,z:hz},{x:sb.x,z:sb.z},{x:b.x,z:b.z}];return pts.filter((p,i)=>!i||Math.hypot(p.x-pts[i-1].x,p.z-pts[i-1].z)>.3)}
function routeLength(pts){let d=0;for(let i=1;i<pts.length;i++)d+=Math.hypot(pts[i].x-pts[i-1].x,pts[i].z-pts[i-1].z);return d}
function makeEmergencyLightBar(g,y=1.82){const redMat=new THREE.MeshBasicMaterial({color:0xff3344}),blueMat=new THREE.MeshBasicMaterial({color:0x3f9dff});const red=new THREE.Mesh(new THREE.BoxGeometry(.48,.12,.30),redMat),blue=new THREE.Mesh(new THREE.BoxGeometry(.48,.12,.30),blueMat);red.position.set(-.25,y,-.1);blue.position.set(.25,y,-.1);g.add(red,blue);g.userData.sirenLights=[red,blue];return g}
function flashEmergencyLights(g,t){const a=g?.userData?.sirenLights;if(!a)return;const flip=Math.floor(t/220)%2===0;a[0].visible=flip;a[1].visible=!flip}
function makeRescueVehicle(){const g=createCarVisual(0xb93431,'van');makeEmergencyLightBar(g,1.82);const bed=new THREE.Mesh(new THREE.BoxGeometry(.75,.13,1.65),new THREE.MeshStandardMaterial({color:0xdedede,roughness:.8}));bed.position.set(.38,.92,.25);g.add(bed);const cross=makeSign('✚','#ffffff');cross.scale.set(.42,.28,.3);cross.position.set(0,1.22,2.14);g.add(cross);return g}
function callFirefighters(){
 if(!state.disease||state.disease.severity<65)return toast('Les pompiers sont réservés ici aux situations graves.');if(emergencyResponse)return toast('Les secours sont déjà engagés.');if(state.interior){toast('🚨 Urgence médicale : les secours organisent ton évacuation vers l’extérieur.');if(state.interior.shopType==='restaurant')restaurantSession=null;leaveInterior();if(state.interior)return toast('Impossible de rejoindre les secours depuis cet intérieur.')}
 const station=nearestCivic('fireStation'),hospital=nearestHospital();if(!station||!hospital)return toast('Aucun service de secours ou hôpital disponible dans cette ville.');const patient={...state.pos},route=emergencyRoute(station,patient),g=makeRescueVehicle();g.position.set(route[0].x,0,route[0].z);scene.add(g);const etaSec=Math.max(8,Math.ceil(routeLength(route)/9.2));emergencyResponse={vehicle:g,phase:'toPatient',route,index:1,speed:9.2,patient,hospital,patientOnBoard:false,dwell:0,statusText:`VSAV en route depuis ${station.name} • ~${etaSec} s`};toast(`🚒 ${station.name} déclenche un VSAV • arrivée estimée ~${etaSec} s`)
}
function removeEmergencyVehicle(){if(emergencyResponse?.vehicle?.parent)scene.remove(emergencyResponse.vehicle);emergencyResponse=null}
function updateEmergencyResponse(dt){const e=emergencyResponse;if(!e?.vehicle)return;flashEmergencyLights(e.vehicle,performance.now());if(e.dwell>0){e.dwell-=dt;if(e.dwell<=0&&e.phase==='pickup'){e.patientOnBoard=true;e.phase='toHospital';e.route=emergencyRoute(e.vehicle.position,e.hospital);e.index=1;e.statusText=`Pris en charge • transport vers ${e.hospital.name}`;toast('🚑 Tu es installé sur le brancard. Direction l’hôpital le plus proche.')}return}
 const target=e.route[e.index];if(!target)return;if(e.patientOnBoard){state.pos.x=e.vehicle.position.x;state.pos.z=e.vehicle.position.z}
 const dx=target.x-e.vehicle.position.x,dz=target.z-e.vehicle.position.z,dist=Math.hypot(dx,dz);if(dist<.35){e.index++;if(e.index>=e.route.length){if(e.phase==='toPatient'){e.phase='pickup';e.dwell=4.5;e.statusText='Les pompiers sont sur place • prise en charge';toast('🚒 Les pompiers arrivent et te prennent en charge.')}else if(e.phase==='toHospital'){const hospital=e.hospital;scene.remove(e.vehicle);emergencyResponse=null;state.hospitalFlow={admitted:true,step:'arrival',viaFirefighters:true};enterInterior('shop',{type:'clinic',displayName:hospital.name,name:hospital.name},{preserveReturn:false});state.pos={x:0,z:5.5};toast(`🏥 Arrivée à ${hospital.name}. Les pompiers te confient à l’équipe médicale.`);openSheet('physicalShop')}return}return}
 const ang=Math.atan2(-dx,-dz);e.vehicle.rotation.y=ang;const step=Math.min(dist,e.speed*dt);e.vehicle.position.x+=dx/dist*step;e.vehicle.position.z+=dz/dist*step
}
...
function policeEmergencyStatus(){if(!policeResponse)return'';if(policeResponse.phase==='toCall')return policeResponse.statusText||'Police-secours en route';if(policeResponse.phase==='scene')return'Police sur place • sécurisation en cours';return''}
function callPoliceEmergency(){
 if(policeResponse)return toast('👮 Une équipe de police-secours est déjà engagée.');
 if(state.interior){toast('📞 17 : l’équipe se rend à l’entrée du bâtiment.');if(state.interior.shopType==='restaurant')restaurantSession=null;leaveInterior();if(state.interior)return toast('Impossible de localiser une sortie pour la police.');}
 const station=nearestCivic('policeStation');if(!station)return toast('Aucun commissariat disponible dans cette ville.');
 const target={...state.pos},route=emergencyRoute(station,target),g=makePoliceResponseVehicle();g.position.set(route[0].x,0,route[0].z);scene.add(g);const speed=10.4,etaSec=Math.max(7,Math.ceil(routeLength(route)/speed));
 policeResponse={vehicle:g,phase:'toCall',route,index:1,speed,target,dwell:0,officers:[],statusText:`Police-secours en route depuis ${station.name} • ~${etaSec} s`,stationName:station.name};
 toast(`🚔 Appel 17 transmis • ${station.name} • arrivée estimée ~${etaSec} s`)
}
function spawnResponseOfficers(r){for(let i=0;i<2;i++){const n=createPerson('police',`dispatch:${Date.now()}:${i}`,r.vehicle.position.x+(i?-.65:.65),r.vehicle.position.z+.45,Math.random);n.dispatchOfficer=true;n.group.rotation.y=r.vehicle.rotation.y;scene.add(n.group);r.officers.push(n)}}
function clearPoliceResponse(){if(!policeResponse)return;if(policeResponse.vehicle?.parent)scene.remove(policeResponse.vehicle);for(const o of policeResponse.officers||[])if(o.group?.parent)scene.remove(o.group);policeResponse=null}
function updatePoliceResponse(dt,t){const r=policeResponse;if(!r?.vehicle)return;flashEmergencyLights(r.vehicle,t);
 if(r.phase==='scene'){
  r.dwell-=dt;const nearby=enemies.filter(n=>n.group?.parent&&Math.hypot(n.group.position.x-r.vehicle.position.x,n.group.position.z-r.vehicle.position.z)<70);
  for(const n of nearby){n.policeFleeUntil=Math.max(n.policeFleeUntil||0,t+3000);n.policeFleeFrom={x:r.vehicle.position.x,z:r.vehicle.position.z}}
  for(let i=0;i<(r.officers||[]).length;i++){const o=r.officers[i],target=nearby[i%Math.max(1,nearby.length)];if(target){const dx=target.group.position.x-o.group.position.x,dz=target.group.position.z-o.group.position.z,d=Math.hypot(dx,dz)||1,k=Math.min(d,dt*2.2);o.group.position.x+=dx/d*k;o.group.position.z+=dz/d*k;setHeading(o,dx,dz)}}
  if(r.dwell<=0){toast('👮 Intervention terminée • la patrouille repart.');clearPoliceResponse()}return
 }
 const target=r.route[r.index];if(!target)return;const dx=target.x-r.vehicle.position.x,dz=target.z-r.vehicle.position.z,dist=Math.hypot(dx,dz);if(dist<.35){r.index++;if(r.index>=r.route.length){r.phase='scene';r.dwell=22;r.statusText='Police sur place • sécurisation en cours';spawnResponseOfficers(r);for(const n of enemies){if(Math.hypot(n.group.position.x-r.vehicle.position.x,n.group.position.z-r.vehicle.position.z)<75){n.policeFleeUntil=t+20000;n.policeFleeFrom={x:r.vehicle.position.x,z:r.vehicle.position.z}}}toast('🚔 Police sur place • les individus hostiles prennent la fuite.');return}return}
 r.vehicle.rotation.y=Math.atan2(-dx,-dz);const step=Math.min(dist,r.speed*dt);r.vehicle.position.x+=dx/dist*step;r.vehicle.position.z+=dz/dist*step
}
...
function updateDisease(dt){
 ...
 const protectedCare=!!(emergencyResponse?.patientOnBoard||state.hospitalFlow?.admitted),factor=protectedCare?.18:1;const sev=state.disease.severity||0;
...
function animate(){
 ...
 const moveSpeed=4.8*needsSpeedMultiplier();if(!busRide&&!emergencyResponse?.patientOnBoard){movePlayer((fx*forward+rx*strafe)*moveSpeed*dt,(fz*forward+rz*strafe)*moveSpeed*dt);updatePlayerTrail()}
...
function phoneHomeHTML(){const fire=emergencyResponse?.statusText||'',pol=policeEmergencyStatus();return `<div class="phoneShell"><div class="phoneTop"><span>StreetPhone</span><b>${formatGameTime(state.timeOfDay)}</b><span>${city().name}</span></div><div class="phoneGrid">${phoneAppTile('phoneMap','🗺️','Carte','Ville & région')}${phoneAppTile('phoneAgenda','📅','Agenda','Rendez-vous')}${phoneAppTile('phoneGuide','📖','Guide','Comment jouer')}${phoneAppTile('phoneDirectory','📍','Annuaire','Autour de moi')}${phoneAppTile('phoneContacts','👤','Contacts','Rencontres')}${phoneAppTile('phoneDialer','☎️','Téléphone','17 / 18')}${phoneAppTile('phoneTransport','🚌','Transports','Bus & train')}</div>${fire||pol?`<div class="phoneEmergencyLive"><b>Intervention en cours</b>${fire?`<small>🚒 ${fire}</small>`:''}${pol?`<small>🚔 ${pol}</small>`:''}</div>`:''}</div>`}
...
function phoneDialerHTML(){const fire=emergencyResponse?.statusText,pol=policeEmergencyStatus();return `${phoneBackButton()}<div class="phoneDialer card"><div class="sectionKicker">APPELS</div><h3>☎️ Composer un numéro</h3><input id="phoneNumber" class="lifeInput phoneNumber" inputmode="numeric" maxlength="3" placeholder="17 ou 18"><div class="phoneKeypad">${['1','2','3','4','5','6','7','8','9','⌫','0','📞'].map(k=>`<button class="phoneKey" data-key="${k}">${k}</button>`).join('')}</div><p class="sub">17 : police-secours • 18 : sapeurs-pompiers</p>${fire?`<div class="phoneEmergencyLive"><small>🚒 ${fire}</small></div>`:''}${pol?`<div class="phoneEmergencyLive"><small>🚔 ${pol}</small></div>`:''}</div>`}
...
function callPhoneNumber(number){const n=String(number||'').replace(/\D/g,'');if(n==='18'){callFirefighters();openSheet('phoneDialer');return}if(n==='17'){callPoliceEmergency();openSheet('phoneDialer');return}toast('📵 Numéro inconnu. Essaie 17 ou 18.')}
...
function menuHTML(){return `<div class="menuHero"><div><div class="sectionKicker">STREETQUEST V22.8</div><h3>${mpNickname()}</h3><p>${city().name} • ${streetCoords()} • ${formatGameTime()}</p></div><button class="avatarMiniBtn" id="menuAvatar">🎨</button></div>
...
function switchCity(id,viaTrain=false){
 removeEmergencyVehicle();clearPoliceResponse();clearTarget(false);busWaitRequest=null;busJourneyPlan=null;busRide=null;currentBoardBusId=null;state.cityId=id;activateCityTransit(id);const sp=city().spawn||{x:2,z:8};state.pos={x:sp.x,z:sp.z};state.yaw=0;state.pitch=0;if(!state.visitedCities.includes(id))state.visitedCities.push(id);
...
function settingsHTML(){return `<div class="card"><div class="sectionKicker">VERSION</div><h3>StreetQuest V22.8</h3><button class="menuBtn full" id="forceUpdate">↻ Vérifier les mises à jour</button></div>
