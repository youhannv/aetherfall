...
function publicPlayer(p){return{id:p.id,name:p.name,city:p.city,x:p.x,z:p.z,yaw:p.yaw,color:p.color}}
...
function joinWorld(socket,data={}){
 ...
 const city=safeText(data.city,20)||'paris',p={id:socket.id,name:safeText(data.name,18)||'Joueur',city,x:safeNum(data.x),z:safeNum(data.z),yaw:safeNum(data.yaw),color:Number(data.color)||0x5fa7d8,lastMove:0};
 players.set(socket.id,p);socket.join(roomName(city));
...
io.on('connection',socket=>{
 ...
 socket.on('disconnect',()=>{const p=players.get(socket.id);if(!p)return;players.delete(socket.id);socket.to(roomName(p.city)).emit('player:left',publicPlayer(p));emitCount(p.city)})
});
